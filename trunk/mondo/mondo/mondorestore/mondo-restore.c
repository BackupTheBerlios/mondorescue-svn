/***************************************************************************
    ...
    cvsid                : $Id: mondo-restore.c,v 1.42 2004/06/21 20:20:37 hugo Exp $
***************************************************************************/
...
//static char cvsid[] = "$Id: mondo-restore.c,v 1.42 2004/06/21 20:20:37 hugo Exp $";
 ...
 */
int let_user_edit_the_mountlist(struct s_bkpinfo *bkpinfo, struct mountlist_itself *mountlist, struct raidlist_itself *raidlist)
{
  int retval=0, res=0;
 ...
 */
bool 
partition_table_contains_Compaq_diagnostic_partition(struct mountlist_itself *mountlist)
{
  int i;
 ...
 */
void 
offer_to_abort_because_Compaq_Proliants_suck(void)
{
  ...
  if (bkpinfo->backup_media_type == iso)
    {
      if (iso_fiddly_bits(bkpinfo, (c=='N')?TRUE:FALSE))
        { log_msg(2, "catchall_mode --- iso_fiddly_bits returned w/ error"); return(1); }
      else
 ...
 */
void
find_pathname_of_executable_preferably_in_RESTORING( char *out_path, char *fname, char*restg )
{
		  ...
		  struct raidlist_itself *raidlist)
{
  ...
  if (g_text_mode)
    {
      if (!ask_me_yes_or_no("Interactive Mode + textonly = experimental! Proceed anyway?"))
...
#ifdef __FreeBSD__
  if (strstr(call_program_and_get_last_line_of_output("cat /tmp/cmdline"), "noresize"))
#else
	...
	{ 
	  ...
	  log_to_screen(" partitioning and formatting will not take place." );
	      ...
	      else
		{
		  ...
		  if (!fmt_errs)
		    {
		      log_to_screen("Errors during disk partitioning were handled OK.");
		      log_to_screen("Partitions were formatted OK despite those errors.");
		      ptn_errs = 0;
		    }
...
// (NB: %s is where your filesystem is mounted now, by default)", MNT_RESTORING);
              if ( popup_and_get_string( "Restore path", "Restore files to where?", tmp, MAX_STR_LEN/4 ) )
                {
		  if ( !strcmp( tmp, "/" ) )
		    { 
		  ...
		  log_msg(1, "Restoring subset");
              ...
              if ( !ask_me_yes_or_no( "Restore another subset of your backup?" ) )
		{ 
		  done = TRUE; 
		}
            }
  ...
  if ( ask_me_yes_or_no( "Label your ext2 and ext3 partitions if necessary?" ) )
    {
      ...
      sprintf( tmp,
	       "label-partitions-as-necessary %s < %s >> %s 2>> %s", g_mountlist_fname, 
	       fstab_fname, MONDO_LOGFILE, MONDO_LOGFILE);
      res=system( tmp );
      if ( res )
	{
	 ...
	 bool nuke_me_please)
{
  ...
  if (iso_fiddly_bits(bkpinfo, nuke_me_please))
    { log_msg(1,"iso_mode --- returning w/ error"); return(1); }
  else
    {
      c=which_restore_mode();
      if (c=='I' || c=='N' || c=='C')
  ...
  if (is_this_device_mounted(MNT_CDROM)) { paranoid_system("umount "MNT_CDROM); }
//  if (! already_mounted)
//    {
	   ...
	   struct raidlist_itself *raidlist )
{
  ...
  /** malloc **/
  char tmp[MAX_STR_LEN], tmpA[MAX_STR_LEN], tmpB[MAX_STR_LEN], tmpC[MAX_STR_LEN];
  ...
  if (!evaluate_mountlist( mountlist, tmpA, tmpB, tmpC ))
    {
      sprintf(tmp, "Mountlist analyzed. Result: \"%s %s %s\" Switch to Interactive Mode?", tmpA, tmpB, tmpC);
  ...
  if ( bkpinfo->differential )
      ...
      else
	{
	  ...
	  g_fprep = fopen("/tmp/prep.sh", "w");
#ifdef __FreeBSD__
	  if (strstr (call_program_and_get_last_line_of_output ("cat /tmp/cmdline"), "nopart"))
#else
	  if (strstr (call_program_and_get_last_line_of_output ("cat /proc/cmdline"), "nopart"))
#endif
	      {
		  log_msg (2, "Not partitioning drives due to 'nopart' option.");
		  res = 0;
	      }
	  else
	      {
  ...
  if ( res )
	...
	{ 
      ...
      else if ( !ask_me_yes_or_no( "Would you like to try to proceed anyway?" ) )
        {
          return( retval );
	}
    }
  ...
  if ( retval ) 
    {
      unmount_all_devices( mountlist ); 
      log_to_screen("Unable to mount all partitions. Sorry, I cannot proceed.");
      return( retval );
    }
  ...
  if (!run_boot_loader( FALSE ))
    {
      log_msg(1, "Great! Boot loader was installed. No need for msg at end.");
...
//  run_program_and_log_output("cp -af /etc/lvm " MNT_RESTORING "/etc/", 1);
  ...
  sprintf( tmp, "label-partitions-as-necessary %s < /tmp/fstab", g_mountlist_fname );
...
after_the_nuke:
  if (retval)
    ...
    {
      log_to_screen("PC was restored successfully. Thank you for using Mondo Rescue.");
      log_to_screen("Please visit http://www.mondorescue.org and thank the dev team.");
    }
  else
    {
#ifdef FREELOADER
 ...
 */
int
restore_to_live_filesystem( struct s_bkpinfo *bkpinfo )
{
  int retval = 0;
...
//  static
  ...
  if (!mountlist || !raidlist) { fatal_error("Cannot malloc() mountlist and/or raidlist"); }
  ...
  if ( IS_THIS_A_STREAMING_BACKUP(bkpinfo->backup_media_type ) )
    {
      log_msg(2, "I probably don't need to unmount or eject the CD-ROM but I'm doing it anyway.");
    }
...
restore_a_biggiefile_from_CD(struct s_bkpinfo *bkpinfo,
			     long bigfileno,
			     struct s_node*filelist, char*pathname_of_last_file_restored)
{
  FILE *fin;
  FILE *fout;
  FILE *fbzip2;
  ...
  /** malloc ***/
  char *checksum, *outfile_fname, *tmp, *bzip2_command, *partimagehack_command, *suffix, *sz_devfile;
  char *bigblk;
  char *p;
  int retval = 0;
  int finished = FALSE;
  ...
  if ( fread((void*)&biggiestruct, 1, sizeof(biggiestruct), fin) < sizeof(biggiestruct))
    {
      log_msg(2, "Warning - unable to get biggiestruct of bigfile #%d", bigfileno+1);
    }
  paranoid_fclose(fin);
  ...
  if ( !checksum[0])
    {
      sprintf(tmp,"Warning - bigfile %ld does not have a checksum",bigfileno+1);
      log_msg(3, tmp);
      p = checksum;
    }
  ...
  if (filelist != NULL)
    {
      ...
      if (!node)
        {
          log_msg(0, "Skipping %s (name isn't in filelist)", biggiestruct.filename);
          pathname_of_last_file_restored[0] = '\0';
	  return (0);
        }
      else if (!(node->selected))
        {
	  log_msg(1, "Skipping %s (name isn't in biggielist subset)", biggiestruct.filename);
          pathname_of_last_file_restored[0] = '\0';
          return( 0 );
        }
    }
  ...
  if (biggiestruct.use_partimagehack)
    {
      if (strncmp ( biggiestruct.filename, "/dev/", 5))
        {
	  log_msg(1, "I was in error when I set biggiestruct.use_partimagehack to TRUE.");
...
//  if (!strncmp ( biggiestruct.filename, "/dev/", 5))
    {
      ...
      log_msg(2, "Calling partimagehack in background because %s is an NTFS /dev entry", outfile_fname);
      sprintf(sz_devfile, "/tmp/%d.%d.000", (int)(random()%32768), (int)(random()%32768));
      mkfifo(sz_devfile, 0x770);
      strcpy(partimagehack_fifo, sz_devfile);
      file_to_openout = partimagehack_fifo;
      switch(pid=fork())
                {
                  case -1: fatal_error("Fork failure");
	          case 0:
		    log_msg(3, "CHILD - fip - calling feed_outfrom_partimage(%s, %s)", biggiestruct.filename, partimagehack_fifo);
		    res=feed_outfrom_partimage(biggiestruct.filename, partimagehack_fifo);
//		    log_msg(3, "CHILD - fip - exiting");
		    exit(res);
		    break;
                  default:
		    log_msg(3, "feed_into_partimage() called in background --- pid=%ld", (long int)(pid));
                }
    }
  else
    {
  ...
  */
  ...
  strncpy(pathname_of_last_file_restored, biggiestruct.filename, MAX_STR_LEN-1);
  ...
  for( sliceno = 1, finished = FALSE; !finished; )
	...
	{
	  log_msg(3, "Cannot find a data slice or terminator slice on CD %d", g_current_media_number);
	  g_current_media_number++;
	  sprintf( tmp, "Asking for %s #%d so that I may read slice #%ld\n",media_descriptor_string(bkpinfo->backup_media_type), g_current_media_number,sliceno);
	  log_msg(2,  tmp );
	  sprintf( tmp, "Restoring from %s #%d", media_descriptor_string(bkpinfo->backup_media_type), g_current_media_number );
	  log_to_screen( tmp );
      ...
      else
	{
	  ...
	  if (does_file_exist(tmp) && length_of_file(tmp) == 0)
	    {
	      log_msg(2, "End of bigfile # %ld (slice %ld is the terminator)", bigfileno + 1, sliceno );
	      finished = TRUE;
              continue;
	    }
	  else
	      ...
	      else
		{
	      ...
	      sprintf( bzip2_command + strlen( bzip2_command ),
		       " -dc %s 2>> %s",
		       slice_fname( bigfileno, sliceno, ARCHIVES_PATH, suffix),
		       MONDO_LOGFILE);
	    }
          else
            {
	  ...
	  sprintf(tmp, "Working on %s #%d, file #%ld, slice #%ld    ", media_descriptor_string(bkpinfo->backup_media_type), g_current_media_number, bigfileno + 1, sliceno );
          log_msg(2, tmp );
	  ...
	  while( !feof( fbzip2 ) )
	      ...
	      if (siz > 0)
		{
		  ...
		  siz = fwrite( bigblk, 1, siz, fout);
                  sprintf(sz_msg + strlen(sz_msg), "; written %ld to fout", siz);
//		  log_msg(2. sz_msg);
		}
	    }
  ...
  if (use_partimage_hack)
    {
      ...
      sprintf(tmp, " ps ax | grep \" partimagehack \" | grep -v grep > /dev/null 2> /dev/null");
      while(system(tmp) == 0)
        { sleep(1); }
 ...
 */
int
restore_a_biggiefile_from_stream(struct s_bkpinfo *bkpinfo,
				 char *orig_bf_fname,
				 long biggiefile_number,
				 char *orig_checksum, //UNUSED
				 long long biggiefile_size, //UNUSED
				 struct s_node *filelist, int use_partimagehack,
				 char*pathname_of_last_file_restored)
{
  ...
  strncpy(pathname_of_last_file_restored, orig_bf_fname, MAX_STR_LEN-1);
  pathname_of_last_file_restored[MAX_STR_LEN-1] = '\0';
  ...
  if (filelist != NULL)
    {
      ...
      if (!node)
        {
          dummy_restore = TRUE;
          log_msg(1, "Skipping big file %ld (%s) - not in biggielist subset", biggiefile_number+1, orig_bf_fname);
          pathname_of_last_file_restored[0] = '\0';
        }
      else if (!(node->selected))
        {
          dummy_restore = TRUE;
	  log_msg(1, "Skipping %s (name isn't in biggielist subset)", orig_bf_fname);
          pathname_of_last_file_restored[0] = '\0';
        }
    }
  ...
  if (use_partimagehack)
    {
      if (strncmp ( orig_bf_fname, "/dev/", 5))
        {
	  log_msg(1, "I was in error when I set use_partimagehack to TRUE.");
  ...
  if (use_partimagehack)
    {
      ...
      use_partimage_hack = TRUE;
      log_msg(2, "Calling partimagehack in background because %s is a /dev entry", outfile_fname);
      sprintf(sz_devfile, "/tmp/%d.%d.000", (int)(random()%32768), (int)(random()%32768));
      mkfifo(sz_devfile, 0x770);
      strcpy(partimagehack_fifo, sz_devfile);
      file_to_openout = partimagehack_fifo;
      switch(pid=fork())
                {
                  case -1: fatal_error("Fork failure");
	          case 0:
		    log_msg(3, "CHILD - fip - calling feed_outfrom_partimage(%s, %s)", outfile_fname, partimagehack_fifo);
		    res=feed_outfrom_partimage(outfile_fname, partimagehack_fifo);
//		    log_msg(3, "CHILD - fip - exiting");
		    exit(res);
		    break;
                  default:
  ...
  else
    {
      if (!strncmp ( orig_bf_fname, "/dev/", 5)) // non-NTFS partition
        ...
        { sprintf( outfile_fname, "%s/%s", bkpinfo->restore_path, orig_bf_fname ); }
      use_partimage_hack = FALSE;
      partimagehack_fifo[0] = '\0';
      file_to_openout = outfile_fname;
      if ( !does_file_exist( outfile_fname ) ) // yes, it looks weird with the '!' but it's correct that way
  ...
  if ( !bkpinfo->zip_exe[0] )
    ...
    {
      sprintf(command,"%s -dc > \"%s\" 2>> %s",bkpinfo->zip_exe, file_to_openout, MONDO_LOGFILE);
    }
      ...
      if ( current_slice_number == 0 )
        {
          ...
          if (!(fin = fopen("/tmp/biggie-blah.txt", "r")))
          ...
          else
            {
              if ( fread( (void*)&biggiestruct, 1, sizeof(biggiestruct), fin) < sizeof(biggiestruct))
                {
                  log_msg(2, "Warning - unable to get biggiestruct of bigfile #%d", biggiefile_number + 1);
                }
              paranoid_fclose(fin);
            }
	}
  ...
  log_msg(1, "pathname_of_last_file_restored is now %s", pathname_of_last_file_restored);
...
//  sprintf(files_to_restore_this_time_fname, "/tmp/ftrttf.%d.%d", (int)getpid(), (int)random());
  ...
  sprintf(temp_log, "/tmp/%d.%d", (int)(random()%32768), (int)(random()%32768));
  
  sprintf(filelist_name, MNT_CDROM"/archives/filelist.%ld",current_tarball_number);
  if ( length_of_file( filelist_name ) <= 2 )
    {
      log_msg(2, "There are _zero_ files in filelist '%s'", filelist_name);
      log_msg(2, "This is a bit silly (ask Hugo to fix mondo-makefilelist, please)");
      log_msg(2, "but it's non-critical. It's cosmetic. Don't worry about it.");
      retval=0;
      goto leave_sub;
    }
  if ( count_lines_in_file(filelist_name) <= 0 || length_of_file( tarball_fname ) <= 0 )
    {
      log_msg(3, "length_of_file(%s) = %ld", tarball_fname, length_of_file(tarball_fname));
      sprintf(tmp,"Unable to restore fileset #%ld (CD I/O error)",current_tarball_number);
      log_to_screen(tmp);
      retval=1;
      goto leave_sub;
    }
  ...
  if (filelist)
    {
      sprintf(filelist_subset_fname, "/tmp/filelist-subset-%ld.tmp", current_tarball_number);
      if ((matches=save_filelist_entries_in_common
      			(filelist_name, filelist, filelist_subset_fname, use_star))
      			<= 0)
	{
	  ...
	  log_msg(1, tmp);
        ...
        {
	  log_msg(3, "Saved fileset %ld's subset to %s", current_tarball_number, filelist_subset_fname);
	}
      sprintf(screen_message, "Tarball #%ld --- %ld matches", current_tarball_number, matches);
  ...
  if (filelist == NULL || matches>0)
    {
      sprintf(xattr_fname, XATTR_LIST_FNAME_RAW_SZ, MNT_CDROM"/archives", current_tarball_number);
      sprintf(acl_fname, ACL_LIST_FNAME_RAW_SZ, MNT_CDROM"/archives", current_tarball_number);
      ...
      if (executable[0])
        {
          ...
          if ( run_program_and_log_output( tmp, FALSE ) )
            {
             log_to_screen("(compare_a_tarball) Compression program not found - oh no!");
             paranoid_MR_finish(1);
            }
...
//      if (strstr(tarball_fname, ".star."))
      if (use_star)
        {
	  sprintf( command, "star -x -force-remove -U " STAR_ACL_SZ " errctl= file=%s", tarball_fname);
	  if (strstr (tarball_fname, ".bz2"))
	    { strcat( command, " -bz"); }
	}
      else
...
#undef BUFSIZE
      sprintf(command+strlen(command), " 2>> %s >> %s", temp_log, temp_log);
      ...
      if (res && length_of_file(temp_log)<5) { res=0; }
      ...
      if (length_of_file(xattr_fname)>0)
        {
          ...
          if (res) { log_to_screen("Errors occurred while setting extended attributes"); }
          else { log_msg(1, "I set xattr OK"); }
          retval += res;
        }
      if (length_of_file(acl_fname)>0)
        {
          ...
          if (res) { log_to_screen("Errors occurred while setting access control lists"); }
          else { log_msg(1, "I set ACL OK"); }
          retval += res;
        }
      if ( retval )
        {
          ...
          system(command);
          log_msg(2, "Errors occurred while processing fileset #%d",current_tarball_number);
        }
      else
        {
  ...
  if (does_file_exist("/PAUSE"))
    { popup_and_OK("Press ENTER to go on. Delete /PAUSE to stop these pauses."); }
...
leave_sub:
  paranoid_free(command); paranoid_free(tmp);
  paranoid_free(filelist_name); paranoid_free(filelist_subset_fname);
  paranoid_free(executable); paranoid_free(temp_log);
  paranoid_free(xattr_fname); paranoid_free(acl_fname);
  log_msg(5, "Leaving");
  return(retval);
}
...
restore_a_tarball_from_stream(struct s_bkpinfo *bkpinfo,
			      ...
			      long current_tarball_number,
			      struct s_node*filelist,
			      long long size, char*xattr_fname, char*acl_fname)
{
  int retval = 0;
  int res = 0;
  ...
  use_star = (strstr(tarball_fname, ".star"))?TRUE:FALSE;
  sprintf( tmp,
	   "Restoring from fileset #%ld (%ld KB) on %s #%d",current_tarball_number,
	   (long) size>>10,
  ...
  sprintf(afio_fname, "/tmp/tmpfs/archive.tmp.%ld", current_tarball_number);
  sprintf(filelist_fname, "%s/filelist.%ld", bkpinfo->tmpdir, current_tarball_number);
  sprintf(filelist_subset_fname, "%s/filelist-subset-%ld.tmp", bkpinfo->tmpdir, current_tarball_number);
//  sprintf(filelist_fname, "/tmp/tmpfs/temp-filelist.%ld", current_tarball_number);
  ...
  if (strstr(tarball_fname, ".star")) { bkpinfo->use_star = TRUE; }
  if (res) { log_msg(1, "Warning - error reading afioball from tape"); }
  ...
  if (!filelist) // if unconditional restore then restore entire fileset
  ...
  else          // If restoring selectively then get TOC from tarball
    {
      ...
      sprintf(command+strlen(command), " > %s 2>> %s", filelist_fname, MONDO_LOGFILE);
      log_msg(1, "command = %s", command);
      if (system(command)) { log_msg(4, "Warning - error occurred while retrieving TOC"); }
      if ((matches=save_filelist_entries_in_common(filelist_fname, filelist, filelist_subset_fname, use_star))
      				 <= 0
        			|| length_of_file(filelist_subset_fname)<2)
        {
	  if (length_of_file(filelist_subset_fname)<2)
	    { log_msg(1, "No matches found in fileset %ld", current_tarball_number); }
	  sprintf(tmp, "Skipping fileset %ld", current_tarball_number);
	  log_msg(2, tmp);
        ...
        {
	  log_msg(5, "%ld matches. Saved fileset %ld's subset to %s", matches, current_tarball_number, filelist_subset_fname);
	  restore_this_fileset = TRUE;
	}
    }
  ...
  if (strstr(tarball_fname, ".star.")) // star
    {
      ...
      if (filelist)
        { sprintf(command+strlen(command), " list=%s", filelist_subset_fname); }
    }
  else                                 // afio
    {
      sprintf( command, "afio -i -M 8m -b %ld %s", TAPE_BLOCK_SIZE, executable);
      if (filelist)
        { sprintf(command+strlen(command), " -w %s", filelist_subset_fname); }
  ...
  if (restore_this_fileset)
    {
      ...
      res=set_fattr_list(filelist_subset_fname, xattr_fname);
      if (res) { log_msg(1, "Errors occurred while setting xattr"); }
      else { log_msg(1, "I set xattr OK"); }
      retval += res;
      
      res=set_acl_list(filelist_subset_fname, acl_fname);
      if (res) { log_msg(1, "Errors occurred while setting ACL"); }
      else { log_msg(1, "I set ACL OK"); }
      retval += res;
 ...
 */
int
restore_all_biggiefiles_from_CD(struct s_bkpinfo *bkpinfo, struct s_node* filelist)
{
  int retval = 0;
  int res;
  ...
  sprintf(biggies_whose_EXATs_we_should_set, "%s/biggies-whose-EXATs-we-should-set", bkpinfo->tmpdir);
  if (!(fbw = fopen(biggies_whose_EXATs_we_should_set, "w")))
   { log_msg(1, "Warning - cannot openout %s", biggies_whose_EXATs_we_should_set); }
  ...
  if ( noof_biggiefiles <= 0 )
    {
      log_msg(2, "OK, no biggiefiles in biggielist; not restoring biggiefiles");
      return(0);
    }
  ...
  for( bigfileno = 0, finished = FALSE; !finished;)
	...
	{
	  log_msg(3, "...but its first slice isn't on this CD. Perhaps this was a selective restore?");
          log_msg(3, "Cannot find bigfile #%ld 's first slice on %s #%d", bigfileno+1, media_descriptor_string(bkpinfo->backup_media_type), g_current_media_number);
          log_msg(3, "Slicename would have been %s", slice_fname(bigfileno+1,0,ARCHIVES_PATH,""));
	  ...
	  if (just_changed_cds)
	    ...
	    {
	      log_msg(2, "There was no bigfile #%ld. That's OK." ,bigfileno+1);
      ...
      else
	{
	  ...
	  update_progress_form(tmp);
	  res = restore_a_biggiefile_from_CD(bkpinfo, bigfileno, filelist, pathname_of_last_biggie_restored);
  ...
  if (fbw)
    {
      ...
      if (length_of_file(xattr_fname)>0 && find_home_of_exe("setfattr"))
        {
 ...
 */
int
restore_all_tarballs_from_CD(struct s_bkpinfo *bkpinfo, struct s_node* filelist)
{
  int retval = 0;
  int res;
  int attempts;
  ...
  log_msg(2, "Insisting on 1st CD, so that I can have a look at LAST-FILELIST-NUMBER");
  ...
  max_val = atol(tmp) + 1;
  sprintf(progress_str,"Restoring from %s #%d", media_descriptor_string(bkpinfo->backup_media_type), g_current_media_number);
  ...
  for(;;)
    {
      ...
      sprintf(tarball_fname, MNT_CDROM"/archives/%ld.afio.bz2",current_tarball_number);
      if (!does_file_exist(tarball_fname)) 
	{
	  sprintf(tarball_fname, MNT_CDROM"/archives/%ld.afio.lzo",current_tarball_number);
	}
      if (!does_file_exist(tarball_fname)) 
	{
	  sprintf(tarball_fname, MNT_CDROM"/archives/%ld.afio.",current_tarball_number);
	}
      if (!does_file_exist(tarball_fname))
	{
	  sprintf(tarball_fname, MNT_CDROM"/archives/%ld.star.bz2", current_tarball_number);
	}
      if (!does_file_exist(tarball_fname))
	{
	  sprintf(tarball_fname, MNT_CDROM"/archives/%ld.star.", current_tarball_number);
	}
      if (!does_file_exist(tarball_fname))
	{
	  ...
	  if (!does_file_exist( MNT_CDROM"/archives/NOT-THE-LAST") || system("find "MNT_CDROM"/archives/slice* > /dev/null 2> /dev/null") == 0 )
	    {
	      break;
	    }
	  g_current_media_number++;
...
//	  log_msg(3, "progress_str = %s", progress_str);
          for(res = 999, attempts = 0; attempts < 3 && res != 0; attempts++)
            {
	      res = restore_a_tarball_from_CD(tarball_fname,current_tarball_number, filelist);
	    }
	    ...
	    { 
          ...
          if (attempts > 1) 
	    { 
	      sprintf( tmp + strlen(tmp)," (%d attempts) - review logs",attempts); 
	    }
          strcpy(comment, tmp);
          if (attempts > 1) 
	    {
 ...
 */
int
restore_all_biggiefiles_from_stream(struct s_bkpinfo *bkpinfo, struct s_node*filelist)
{
...
// get xattr and acl files if they're there
  ...
  if (ctrl_chr == BLK_START_EXTENDED_ATTRIBUTES)
    {
      res = read_EXAT_files_from_tape(bkpinfo, &biggie_size, biggie_fname, &ctrl_chr, xattr_fname, acl_fname); 
    }
  ...
  for( res = read_header_block_from_stream( &biggie_size, biggie_fname, &ctrl_chr); ctrl_chr!=BLK_STOP_BIGGIEFILES; res=read_header_block_from_stream(&biggie_size, biggie_fname, &ctrl_chr))
    {
      if (ctrl_chr != BLK_START_A_NORMBIGGIE && ctrl_chr != BLK_START_A_PIHBIGGIE) 
	{
      ...
      else
	{
      ...
      res = restore_a_biggiefile_from_stream(bkpinfo,biggie_fname,
					     ...
					     filelist, ctrl_chr,
					     pathname_of_last_biggie_restored);
      log_msg(1, "I believe I have restored %s", pathname_of_last_biggie_restored);
      if (fbw && pathname_of_last_biggie_restored[0])
        { fprintf(fbw, "%s\n", pathname_of_last_biggie_restored); }
      retval += res;
      current_bigfile_number++;
  ...
  if (current_bigfile_number != noof_biggiefiles && noof_biggiefiles != 0 )
    {
      sprintf( tmp,
	       "Warning - bigfileno=%ld but noof_biggiefiles=%ld\n",current_bigfile_number,
	       noof_biggiefiles);
    }
  else
    {
      sprintf( tmp,"%ld biggiefiles in biggielist.txt; %ld biggiefiles processed today.",
	       noof_biggiefiles,
	       current_bigfile_number);
    }
  log_msg(1, tmp);
  ...
  if (fbw)
    {
	  ...
	  iamhere("Setting biggie-EXATs");
          if (length_of_file(acl_fname)  >0)
            {
	      log_msg(1, "set_acl_list(%s,%s)", biggies_whose_EXATs_we_should_set, acl_fname);
	      set_acl_list  (biggies_whose_EXATs_we_should_set, acl_fname);
	    }
          if (length_of_file(xattr_fname)>0)
            {
	      log_msg(1, "set_fattr_List(%s,%s)", biggies_whose_EXATs_we_should_set, xattr_fname);
	      set_fattr_list(biggies_whose_EXATs_we_should_set, xattr_fname); 
	    }
	}
      else
        {
	  iamhere("No biggiefiles selected. So, no biggie-EXATs to set.");
	}
    }
  if (does_file_exist("/PAUSE"))
    { popup_and_OK("Press ENTER to go on. Delete /PAUSE to stop these pauses."); }
 ...
 */
int
restore_all_tarballs_from_stream(struct s_bkpinfo *bkpinfo, struct s_node *filelist)
{
  int retval = 0;
  int res;
  ...
  while(ctrl_chr!=BLK_STOP_AFIOBALLS)
    {
      update_progress_form(progress_str);
      sprintf(xattr_fname, "%s/xattr-subset-%ld.tmp", bkpinfo->tmpdir, current_afioball_number);
      sprintf(acl_fname, "%s/acl-subset-%ld.tmp", bkpinfo->tmpdir, current_afioball_number);
      unlink(xattr_fname);
      unlink(acl_fname);
      if (ctrl_chr == BLK_START_EXTENDED_ATTRIBUTES)
        {
	  iamhere("Reading EXAT files from tape");
	  res = read_EXAT_files_from_tape(bkpinfo, &tmp_size, tmp_fname, &ctrl_chr, xattr_fname, acl_fname);
        }
      if (ctrl_chr != BLK_START_AN_AFIO_OR_SLICE)
        { wrong_marker(BLK_START_AN_AFIO_OR_SLICE, ctrl_chr); }
      sprintf(tmp,
	      ...
	      tmp_fname,
	      (long) tmp_size>>10);
      res = restore_a_tarball_from_stream(bkpinfo,tmp_fname,current_afioball_number,filelist,tmp_size, xattr_fname, acl_fname);
      retval += res;
      if ( res )
        {
	  sprintf(tmp,"Fileset %ld - errors occurred",current_afioball_number);
	  log_to_screen(tmp);
	}
      res = read_header_block_from_stream(&tmp_size, tmp_fname, &ctrl_chr);
      if (ctrl_chr != BLK_STOP_AN_AFIO_OR_SLICE)
	{
      ...
      current_afioball_number++;
      g_current_progress++;
      sprintf(progress_str,"Restoring from fileset #%ld on %s #%d",current_afioball_number, media_descriptor_string(bkpinfo->backup_media_type), g_current_media_number);      
      res=read_header_block_from_stream(&tmp_size, tmp_fname, &ctrl_chr);
      unlink(xattr_fname);
      unlink(acl_fname);
    } // next
 ...
 */
int
restore_everything(struct s_bkpinfo*bkpinfo,
		   struct s_node *filelist)
{
  int resA;
  int resB;
  ...
  if (!find_home_of_exe("petris") && !g_text_mode)
    {
      newtDrawRootText(0, g_noof_rows-2, "Press ALT-<left cursor> twice to play Petris :-) ");
      newtRefresh();
    }
  ...
  if ( length_of_file("/etc/raidtab") > 0 )
    {
      ...
      run_program_and_log_output("cp -f /etc/raidtab " MNT_RESTORING "/etc/raidtab", FALSE);
    }
  kill_petris();
     ...
     /* NB: bkpinfo hasn't been populated yet, except for ->tmp which is "/tmp" */
{
  ...
  if (!(fout = fopen(datadisks_fname, "w"))) { log_OS_error(datadisks_fname); return(1); }
  for (i = 0; i < 32; i++)
    {
      for (j = 0; j < 4; j++)
        {
  ...
  sprintf( command,
	   "cat %s | grep -x \"%s.*\" > %s",
	   g_filelist_full,
	   restore_this_directory,
	   g_filelist_full);
  if (system(command))
    {
      retval++;
      log_to_screen("Error(s) occurred while processing filelist and wildcard");
    }
  iamhere("FIXME");
  fatal_error("This will fail");
  sprintf(command,
  ...
  if (system(command))
    {
      log_msg(1, "Error(s) occurred while processing biggielist and wildcard");
    }
...
void welcome_to_mondorestore()
{
  ...
  log_msg(0, "DON'T PANIC! Mondorestore logs almost everything, so please " );
  log_msg(0, "don't break out in a cold sweat just because you see a few  " );
  log_msg(0, "error messages in the log. Read them; analyze them; see if  " );
  log_msg(0, "they are significant; above all, verify your backups! Please" );
  log_msg(0, "attach a compressed copy of this log to any e-mail you send " );
  log_msg(0, "to the Mondo mailing list when you are seeking technical    " );
  log_msg(0, "support. Without it, we can't help you.               - Hugo" );
  log_msg(0, "------------------------------------------------------------" );
  log_msg(0, "BTW, despite (or perhaps because of) the wealth of messages," );
  log_msg(0, "some users are inclined to stop reading this log.  If Mondo " );
  log_msg(0, "stopped for some reason, chances are it's detailed here.    " );
  log_msg(0, "More than likely there's a message at the very end of this  " );
  log_msg(0, "log that will tell you what is wrong.  Please read it!      " );
  log_msg(0, "------------------------------------------------------------");
}
...
/* Configure global variables */
#ifdef __FreeBSD__
  if (strstr(call_program_and_get_last_line_of_output("cat /tmp/cmdline"), "textonly"))
#else
  if (strstr(call_program_and_get_last_line_of_output("cat /proc/cmdline"), "textonly"))
#endif
    { g_text_mode = TRUE; log_msg(1, "TEXTONLY MODE"); }
  else
    { g_text_mode = FALSE; }               // newt :-)
  if ( ! ( bkpinfo = g_bkpinfo_DONTUSETHIS = malloc( sizeof(struct s_bkpinfo))))
  ...
  if (am_I_in_disaster_recovery_mode())
    { run_program_and_log_output("mount / -o remount,rw",2); } // for b0rken distros
  g_main_pid = getpid();
  ...
  if (strlen(g_mountlist_fname) < 3)
    { fatal_error("Serious error in malloc()'ing. Could be a bug in your glibc."); }
...
/* Backup original mountlist.txt */
  ...
  if (!does_file_exist(g_mountlist_fname))
    {
      log_msg(2, "%ld: Warning - g_mountlist_fname (%s) does not exist yet", __LINE__, g_mountlist_fname);
    }
  else if ( !does_file_exist(tmp) )
    {
       sprintf(tmp, "cp -f %s %s.orig", g_mountlist_fname, g_mountlist_fname);
       run_program_and_log_output(tmp, FALSE);
    }
...
/* Init directories */
  ...
  run_program_and_log_output( "umount "MNT_CDROM, FALSE );
  run_program_and_log_output( "ln -sf /var/log/mondo-archive.log /tmp/mondo-restore.log", FALSE );
...
#endif
      load_raidtab_into_raidlist( raidlist, RAIDTAB_FNAME );
      if (!does_file_exist(g_mountlist_fname)) { strcpy(g_mountlist_fname, "/tmp/mountlist.txt"); }
  ...
  if (argc == 5 && strcmp( argv[1], "--common") == 0 )
    {
      ...
      if (!filelist) { fatal_error("Failed to load filelist"); }
...
//      toggle_path_selection (filelist, "/root/stuff", TRUE);
      ...
      res = save_filelist_entries_in_common(
      			a, filelist, b, FALSE);
      free_filelist(filelist);
      printf("res = %d", res);
      finish(0);
  ...
  if (argc == 3 && strcmp( argv[1], "--paa" ) == 0 )
    ...
    { // live!
      if (argc!=1)
        {
	  popup_and_OK("Live mode doesn't support command-line parameters yet.");
	  paranoid_MR_finish(1);
//	  return(1);
	}
      log_msg (1, "I am in normal, live mode.");
...
/* Disaster recovery mode (must be) */
      ...
      iamhere("Returned from calling load_mountlist and load_raidtab successfully");

      if (argc>1 && (strcmp( argv[1], "--compare" ) == 0 || strcmp( argv[1], "--nuke" ) == 0 ))
        {
      ...
      if ( retval ) 
        { 
          log_to_screen( "Warning - load_raidtab_into_raidlist returned an error" ); 
        }
  ...
  if (retval)
	...
	{ 
          log_to_screen( "See /tmp/changed.files for list of files that have changed." );
	}
      mvaddstr_and_log_it ( g_currentY++,
			0,
      ...
      if (!system("which post-nuke > /dev/null 2> /dev/null"))
        {
          ...
          if (mount_all_devices(mountlist, TRUE))
          ...
          else
            {
              ...
              if ((res=system(tmp))) { log_OS_error(tmp); }
