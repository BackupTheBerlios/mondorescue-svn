...
//      fatal_error("Bad marker"); // return(1);
	...
	if (strcmp(temp_cksum, actual_cksum)) {
		asprintf(&tmp, _("actual cksum=%s; recorded cksum=%s"), actual_cksum,
				 temp_cksum);
		log_to_screen(tmp);
		paranoid_free(tmp);

		asprintf(&tmp, _("%s (%ld K) is corrupt on tape"), temp_fname,
				 (long) orig_size >> 10);
		log_to_screen(tmp);
		paranoid_free(tmp);
		retval++;
...
int write_data_disks_to_stream(char *fname)
{
	...
	if (!does_file_exist(fname)) {
		asprintf(&tmp, _("Cannot find %s"), fname);
		log_to_screen(tmp);
		paranoid_free(tmp);
		return (1);
	}
...
void wrong_marker(int should_be, int it_is)
{
	...
	/*@ end vars *************************************************** */
	asprintf(&tmp, _("Wrong marker! (Should be %s, is actually %s)"),
