/***************************************************************************
    ...
    cvsid                : $Id: mondo-rstr-compare.c,v 1.1 2004/06/10 15:29:13 hugo Exp $
***************************************************************************/
...
//static char cvsid[] = "$Id: mondo-rstr-compare.c,v 1.1 2004/06/10 15:29:13 hugo Exp $";
 ...
 */
int 
compare_a_biggiefile(struct s_bkpinfo *bkpinfo, long bigfileno)
{
  ...
  if ( !does_file_exist( slice_fname(bigfileno, 0, ARCHIVES_PATH, "" ) ) )
	...
	{
	  sprintf(tmp_ptr, "No CD's left. No biggiefiles left. No prob, Bob." );
	  log_msg(2,  tmp_ptr );
	  return( 0 );
        }
    }
  ...
  if ( system( command_ptr ) )
      ...
      else
	{
	  ...
	  paranoid_fclose( fin );
	  for(i = strlen( original_cksum_ptr ); i > 0 && original_cksum[i-1] < 32; i-- );
	  original_cksum[i] = '\0';
  ...
  sprintf( tmp_ptr, "bigfile #%ld ('%s') ", bigfileno + 1, bigfile_fname_ptr);
 ...
 */
int 
compare_all_biggiefiles(struct s_bkpinfo*bkpinfo)
{
  int retval = 0;
  int res;
  ...
  if ( length_of_file(BIGGIELIST) < 6 )
    { 
      log_msg(1, "OK, really teeny-tiny biggielist; not comparing biggiefiles" ); 
      return( 0 ); 
    }
  ...
  open_progress_form("Comparing large files", "I am now comparing the large files" , "against the filesystem. Please wait.", 
		     "", 
		     noof_biggiefiles );
 ...
 */
int 
compare_a_tarball( char *tarball_fname, int current_tarball_number )
{
  int retval = 0;
  int res;
  long noof_lines;
  ...
  /***  needs malloc *********/
  char *command, *tmp, *filelist_name, *logfile, *archiver_exe, *compressor_exe;
  ...
  sprintf( filelist_name, MNT_CDROM"/archives/filelist.%d", current_tarball_number );
  ...
  if (compressor_exe[0])
    {
	  ...
	  else { fatal_error("(compare_a_tarball) Please use only bzip2 with star"); }
	}
      else      // afio
        {
...
#undef BUFSIZE
  ...
  sprintf(tmp, "%ld difference%c in fileset #%d          ",
	  archiver_errors, (archiver_errors != 1) ? 's' : ' ', current_tarball_number);
  if (archiver_errors)
    {
      sprintf(tmp,
 ...
 */
int 
compare_all_tarballs( struct s_bkpinfo *bkpinfo )
{
  int retval = 0;
  int res;
  ...
  max_val = atol( tmp );
  sprintf( progress_str, "Comparing with %s #%d ", media_descriptor_string(bkpinfo->backup_media_type), g_current_media_number );
  ...
  for( ;; )
    {
      ...
      sprintf( tarball_fname,
	       MNT_CDROM"/archives/%d.afio.bz2",
	       current_tarball_number);
      
      if ( !does_file_exist( tarball_fname ) ) 
	{
	  sprintf( tarball_fname, MNT_CDROM"/archives/%d.afio.lzo", current_tarball_number );
	}
      if (!does_file_exist ( tarball_fname ) ) 
	{
	...
	{
	  if ( !does_file_exist( MNT_CDROM"/archives/NOT-THE-LAST" ) || 
	       system( "find "MNT_CDROM"/archives/slice* > /dev/null 2> /dev/null" )
	       == 0 )
	    {
	...
	{
	  res = 
	    compare_a_tarball( tarball_fname, current_tarball_number );
 ...
 */
int 
compare_to_CD( struct s_bkpinfo *bkpinfo )
{
  /** needs malloc *********/
  ...
  if ( count_lines_in_file("/tmp/changed.txt") > 0 )
    {
	  ...
	  log_to_screen( "The differences were logfiles and temporary files. Your archives are fine.");
	}
      else
        {
	  q = count_lines_in_file("/tmp/changed.files");
	  sprintf( tmp, "%ld significant difference%s found.", q, (q!=1) ? "s" : "");
	  mvaddstr_and_log_it( g_currentY++, 0, tmp);
	  log_to_screen (tmp);
    ...
    {
      log_to_screen( "No significant differences were found. Your backup is perfect." );
    }
  kill_petris();
  paranoid_free(tmp);
  return( retval );
 ...
 */
int
compare_to_cdstream(struct s_bkpinfo *bkpinfo)
{
  int res;
  ...
  mvaddstr_and_log_it(g_currentY,
		      0,
		      "Verifying archives against filesystem" );
  ...
  chdir( dir );
  if ( length_of_file ( "/tmp/changed.txt" ) > 2 && length_of_file ( "/tmp/changed.files" ) > 2 )
    {
      log_msg(0, "Type 'less /tmp/changed.files' to see which files don't match the archives" );
      log_msg(2, "Calling popup_changelist_from_file()" );
      popup_changelist_from_file( "/tmp/changed.files" );
 ...
 **************************************************************************/
int 
compare_to_tape( struct s_bkpinfo *bkpinfo )
{
  int res;
  char *dir, *command;
