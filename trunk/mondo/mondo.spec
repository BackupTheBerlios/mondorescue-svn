...
%ifarch ia64
Requires:	mindi >= 1.02, bzip2 >= 0.9, afio, mkisofs, binutils, elilo, newt >= 0.50, slang >= 1.4.1, buffer, parted
%{!?_without_xmondo:BuildRequires:    gcc-c++, XFree86-devel, qt-devel, kdelibs-devel, libpng-devel}
%else
Requires:	mindi >= 1.02, bzip2 >= 0.9, afio, mkisofs, binutils, syslinux >= 1.52, newt >= 0.50, slang >= 1.4.1, cdrecord, buffer
...
utilsateur de Linux pour cr�r un CD de sauvegarde/restauration
...
for fname in mondo/mondoarchive/.libs/mondoarchive mondo/mondorestore/.libs/mondorestore ; do
    %{__install} -m 755 $fname $RPM_BUILD_ROOT%{_sbindir}
    %{__install} -m 755 $fname $RPM_BUILD_ROOT%{_datadir}/mondo
done
%{!?_without_xmondo:%{__install} -m 755 mondo/xmondo/.libs/xmondo $RPM_BUILD_ROOT%{_sbindir}}
...
# Hugo's way
#        %{__install} -m 755 $fname $RPM_BUILD_ROOT%{_libdir}
# ----------
# Joshua's way
