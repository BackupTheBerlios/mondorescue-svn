/* libmondo-stream.c
   $Id: libmondo-stream.c,v 1.4 2004/06/21 20:20:36 hugo Exp $
...
/*@unused@*/
//static char cvsid[] = "$Id: libmondo-stream.c,v 1.4 2004/06/21 20:20:36 hugo Exp $";
...
*/
  ...
  for(i=0; i<g_tapecatalog->entries; i++)
    {
      log_it("i=%d type=%s num=%d aux=%ld posK=%lld", i, (g_tapecatalog->el[i].type==fileset)?"fileset":"bigslice", g_tapecatalog->el[i].number, g_tapecatalog->el[i].aux, g_tapecatalog->el[i].tape_posK);
    }
  //  if (!bkpinfo->please_dont_eject)
  //    { eject_device(bkpinfo->media_device); }
  paranoid_free(blk);
...
// xattr
          ...
          if(*pctrl_chr!=BLK_START_EXAT_FILE){wrong_marker(BLK_START_EXAT_FILE,*pctrl_chr);}
	  if (!strstr(tmp_fname, "xattr")) { fatal_error("Wrong order, sunshine."); }
	  read_file_from_stream_to_file(bkpinfo, xattr_fname, *ptmp_size);
	  res = read_header_block_from_stream(ptmp_size, tmp_fname, pctrl_chr);
          if(*pctrl_chr!= BLK_STOP_EXAT_FILE) {wrong_marker(BLK_STOP_EXAT_FILE,*pctrl_chr);}
// acl
	  res = read_header_block_from_stream(ptmp_size, tmp_fname, pctrl_chr);
	  if (!strstr(tmp_fname, "acl")) { fatal_error("Wrong order, sunshine."); }
          if(*pctrl_chr!=BLK_START_EXAT_FILE){wrong_marker(BLK_START_EXAT_FILE,*pctrl_chr);}
...
int write_EXAT_files_to_tape(struct s_bkpinfo *bkpinfo, char*xattr_fname, char*acl_fname)
{
  int res=0;
// EXATs
              write_header_block_to_stream(length_of_file(xattr_fname), xattr_fname, BLK_START_EXTENDED_ATTRIBUTES);
// xattr
              write_header_block_to_stream(length_of_file(xattr_fname), xattr_fname, BLK_START_EXAT_FILE);
	      write_file_to_stream_from_file(bkpinfo, xattr_fname);  
	      write_header_block_to_stream(-1, xattr_fname, BLK_STOP_EXAT_FILE);
// acl
              write_header_block_to_stream(length_of_file(acl_fname), acl_fname, BLK_START_EXAT_FILE);
	      write_file_to_stream_from_file(bkpinfo, acl_fname);  
	      write_header_block_to_stream(-1, acl_fname, BLK_STOP_EXAT_FILE);
              write_header_block_to_stream(length_of_file(xattr_fname), xattr_fname, BLK_STOP_EXTENDED_ATTRIBUTES);
  return(res);
}
  ...
  if (g_current_media_number != tapeno)
    {
      ...
      sprintf(tmp, "When the tape drive goes quiet, please insert volume %d in this series.", tapeno);
      popup_and_OK (tmp);
...
{
  long long final_alleged_writeK, final_projected_certain_writeK, final_actually_certain_writeK=0, cposK, bufsize_K;
  int last, curr, i;
  ...
  if ((p=strrchr(latest_fname, '.'))) { strcpy(suffix, ++p); } else { suffix[0] = '\0'; }
  if (strstr(latest_fname, ".afio.") || strstr(latest_fname, ".star.")) { type = fileset; }
  else if (strstr(latest_fname, "slice")) { type = biggieslice; }
  else
    {
      log_it("fname = %s", latest_fname);
      fatal_error("Unknown type. Internal error in maintain_collection_of_recent_archives()");
    }
  mkdir(tmpdir, 0x700);
  sprintf(command, "cp -f %s %s", latest_fname, tmpdir);
  if (run_program_and_log_output(command, 6)) { log_it("Warning - failed to copy %s to backcatalog at %s", latest_fname, tmpdir); }
  ...
  if ( curr<0 )
    {
      iamhere( "Not far enough into tape to start deleting old archives from collection." );
      return( 0 );
    }
...
int set_tape_block_size_with_mt(char*tapedev, long internal_tape_block_size)
{
  char *tmp;
  int res;
  ...
  if (strncmp(tapedev, "/dev/", 5))
    {
      log_msg(1, "Not using 'mt setblk'. This isn't an actual /dev entry.");
      return(0);
    }
  malloc_string( tmp );
...
openin_tape (struct s_bkpinfo *bkpinfo)
{
  ...
  if (!(g_tapecatalog = malloc(sizeof(struct s_tapecatalog)))) { fatal_error("Cannot alloc mem for tape catalog"); }
  ...
  set_tape_block_size_with_mt(bkpinfo->media_device, bkpinfo->internal_tape_block_size);

//  start_buffer_process( bkpinfo->media_device, g_tape_fifo, FALSE);
  log_it ("Opening IN tape");
  if (!(g_tape_stream = open_device_via_buffer(bkpinfo->media_device, 'r', bkpinfo->internal_tape_block_size))) { log_OS_error(g_tape_fifo); log_to_screen ("Cannot openin stream device"); return(1); }
 ...
 */
int
openout_tape (char *tapedev, long internal_tape_block_size)
{
//  char sz_call_to_buffer[MAX_STR_LEN];
  ...
  if (!(g_tape_stream = open_device_via_buffer(tapedev, 'w', internal_tape_block_size))) { log_OS_error(g_tape_fifo); log_to_screen ("Cannot openin stream device"); return(1); }
  return(0);
}
...
//  long bytes_successfully_read_in_this_time = 0;
  ...
  if (foutstream) { fout = foutstream; }
  else { fout = fopen (outfname, "w"); }
  if (!fout)
    {
      log_OS_error (outfname);
  ...
  for (noof_blocks = 0; size > 0; noof_blocks++, size-=bytes_to_write, total_read_from_tape_for_this_file+=bytes_read)
    {
      bytes_to_write = (size < TAPE_BLOCK_SIZE) ? (long)size : TAPE_BLOCK_SIZE;
...
//	  crctt=crc16=0;
          ...
          log_msg(4, "where_I_was_... = %lld", where_I_was_before_tape_change);
	  ...
	  log_msg(4, "Skipped irrelevant files OK.");
          for(size = orig_size; size > where_I_was_before_tape_change; size-=bytes_to_write)
	    {
	      bytes_read = fread(datablock, 1, bytes_to_read, g_tape_stream);
	    }
	  log_msg(4, "'size' is now %lld (should be %lld)", size, where_I_was_before_tape_change);
  ...
  log_msg(6, "Total read from tape for this file = %lld", total_read_from_tape_for_this_file);
...
*/
  ...
  tempblock[i] = '\0';
  log_msg(6, "%s (fname=%s, size=%ld K)", marker_to_string (*pcontrol_char),
...
int register_in_tape_catalog(t_archtype type, int number, long aux, char*fn)
{
  ...
  if (last >= MAX_TAPECATALOG_ENTRIES)
    {
      log_it("Warning - can't log #%d in tape catalog - too many entries already", number);
      return(-1);
    }
...
int skip_incoming_files_until_we_find_this_one(char*the_file_I_was_reading)
{
  ...
  pA = strrchr(temp_fname, '/'); if (pA) { pA++; } else { pA = temp_fname; }
  pB = strrchr(the_file_I_was_reading, '/'); if (pB) { pB++; } else { pB = the_file_I_was_reading; }
  while (strcmp(pA, pB))
    {
...
start_to_read_from_next_tape (struct s_bkpinfo *bkpinfo)
{
  ...
  if (ctrlchr != BLK_START_OF_TAPE) { wrong_marker (BLK_START_OF_TAPE, ctrlchr); }
  res+=read_header_block_from_stream (&temp_size, sz_msg, &ctrlchr);	/* just in case */
  if (ctrlchr != BLK_START_OF_BACKUP) { wrong_marker (BLK_START_OF_BACKUP, ctrlchr); }
  else { log_msg(3, "Next tape opened OK. Whoopee!"); }
...
start_to_write_to_next_tape (struct s_bkpinfo *bkpinfo)
{
  ...
  if (bkpinfo->backup_media_type == cdstream)
    {
      sprintf (command, "cdrecord -eject dev=%s speed=%d fs=24m -waiti - >> %s 2>> %s", bkpinfo->media_device, bkpinfo->cdrw_speed, MONDO_LOGFILE, MONDO_LOGFILE);
  ...
  else
    {
      ...
      if (!(g_tape_stream = open_device_via_buffer(bkpinfo->media_device, 'w', bkpinfo->internal_tape_block_size)))
	{
...
int write_backcatalog_to_tape (struct s_bkpinfo *bkpinfo)
{
  ...
  for(i=0; i<=last; i++)
    {
      sprintf(fname, "%s/tmpfs/backcatalog/%s", bkpinfo->tmpdir, g_tapecatalog->el[i].fname);
      if (!does_file_exist(fname))
        { log_msg(6, "Can't write %s - it doesn't exist.", fname); }
      else
        {
          write_header_block_to_stream (length_of_file(fname), "start-backcatalog-afio-or-slice", BLK_START_AN_AFIO_OR_SLICE);
          log_msg(2, "Writing %s", fname);
          if (write_file_to_stream_from_file(bkpinfo, fname))
	    { res++; log_msg(2, "%s failed", fname); }
	  if (i!=last)
	    { write_header_block_to_stream (0, "stop-backcatalog-afio-or-slice", BLK_STOP_AN_AFIO_OR_SLICE); }
        }
    }
  paranoid_free(fname);
...
write_file_to_stream_from_file (struct s_bkpinfo *bkpinfo, char *infile)
{
  ...
  if (infile_basename) { infile_basename++; } else { infile_basename=infile; }
  ...
  sprintf (tmp, "Writing file '%s' to tape (%ld KB)", p,
	   (long) filesize >> 10);
  log_it (tmp);
  write_header_block_to_stream (filesize, infile_basename, BLK_START_FILE);
//go_here_to_restart_saving_of_file:
  if (!(fin = fopen (infile, "r"))) { log_OS_error(infile); return(1); }
  for (noof_blocks = 0; filesize > 0;
      ...
      else
	{
      ...
      g_tape_posK += fwrite (datablock, 1, /*bytes_to_read */ (size_t) TAPE_BLOCK_SIZE,
			     g_tape_stream) / 1024;
      if (g_sigpipe)
        {
