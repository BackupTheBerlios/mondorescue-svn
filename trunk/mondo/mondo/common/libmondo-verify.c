...
verify_afioballs_on_CD (struct s_bkpinfo *bkpinfo, char *mountpoint)
{
  ...
  sprintf (tmp, "Verifying %s #%d's tarballs", media_descriptor_string(bkpinfo->backup_media_type), g_current_media_number);
  open_evalcall_form (tmp);
  
  for (total_sets = set_number;
       does_file_exist (vfy_tball_fname (bkpinfo, mountpoint, total_sets));
       total_sets++) { log_msg(1, "total_sets = %d", total_sets); }
...
//  long long outlen;
  ...
  if (!bufblkA) { if (!(bufblkA = malloc(maxbufsize))) { fatal_error("Cannot malloc bufblkA"); } }
  if (!bufblkB) { if (!(bufblkB = malloc(maxbufsize))) { fatal_error("Cannot malloc bufblkB"); } }
  ...
  iamhere("before vsbf");
  sprintf (tmp, "Verifying %s#%d's big files", media_descriptor_string(bkpinfo->backup_media_type), g_current_media_number);
  open_evalcall_form (tmp);
  iamhere("after vsbf");
  ...
  while (does_file_exist
 	(slice_fname(bigfile_num, slice_num, mountpoint, bkpinfo->zip_suffix))
	||
	does_file_exist
...
// handle slices until end of CD
      if (slice_num == 0)
	{
	  ...
	  sprintf( tmp, "%s/%s", bkpinfo->restore_path, biggiestruct.filename );
	  log_msg(2, "Opening biggiefile #%ld - '%s'", bigfile_num, tmp);
	  if (!(forig = fopen(tmp, "r"))) { log_msg(2, "Failed to open bigfile. Darn."); retval++; }
	  slice_num++;
	}
      ...
      else
	{
	  ...
	  if (bkpinfo->compression_level > 0)
	    { sprintf(command, "cat %s | %s -dc 2>> %s", slice_fname(bigfile_num, slice_num, mountpoint, bkpinfo->zip_suffix), sz_exe, MONDO_LOGFILE); }
	  else
	    { sprintf(command, "cat %s", slice_fname(bigfile_num, slice_num, mountpoint, bkpinfo->zip_suffix)); }
 	  if ((pin = popen(command,"r")))
	    {
		  ...
		  if (currsizA != currsizB) { res++; }
		  else
		    {
		      for(j=0; j<currsizA && bufblkA[j]==bufblkB[j]; j++);
		      if (j<currsizA) { res++;}
		    }
		}
	      paranoid_pclose(pin);
	      if (res && !strncmp(biggiestruct.filename," /dev/", 5))
	        {
		  log_msg(3, "Ignoring differences between %s and live filesystem because it's a device and therefore the archives are stored via partimagehack, not dd.", biggiestruct.filename);
	  	  log_msg(3, "If you really want verification for %s, please contact the devteam and offer an incentive.", biggiestruct.filename);
		  res=0;
		}
	      if (res)
	        {
		  log_msg(0, "afio: \"%s\": Corrupt biggie file, says libmondo-archive.c", biggiestruct.filename);
		  retval++;
		}
	    }
	  slice_num++;
  ...
  close_evalcall_form ();
  if (bufblkA) { paranoid_free(bufblkA); }
  if (bufblkB) { paranoid_free(bufblkB); }
  paranoid_free(tmp);
  paranoid_free(command);
 ...
 */
int
verify_a_tarball (struct s_bkpinfo *bkpinfo, char *tarball_fname)
{
	/*@ buffers **********************************************************/
  char *command;
	char *outlog;
  ...
  if (strstr(tarball_fname, ".star"))
    {
      ...
      sprintf (command, "star -diff diffopts=mode,size,data file=%s %s >> %s 2>> %s",
	       tarball_fname,
	       (strstr(tarball_fname, ".bz2"))?"-bz":" ",
	       outlog, outlog);
    }
  else
    {
	       ...
	       fgets (tmp, MAX_STR_LEN, pin))
	    {
	      if (bkpinfo->use_star)
	          ...
	          if (!strstr(tmp, "diffopts="))
		    {
		      ...
		      if (strchr(tmp, '/') )
		        {
		          if (!diffs) { log_msg (0, "'%s' - differences found", tarball_fname); }
...
/*  log_it(tmp); */
  ...
  if (biggie_cksum[0] != '\0')
      ...
      if (strcmp (biggie_cksum, orig_cksum))
	{
	  ...
	  log_to_screen (tmp);
	  sprintf(tmp, "echo \"%s\" >> /tmp/biggies.changed", biggie_fname);
	  system(tmp);
	}
    }
...
verify_afioballs_from_stream (struct s_bkpinfo *bkpinfo)
{
  ...
  sprintf(curr_xattr_list_fname, XATTR_BIGGLST_FNAME_RAW_SZ, bkpinfo->tmpdir);  
  sprintf(curr_acl_list_fname,     ACL_BIGGLST_FNAME_RAW_SZ, bkpinfo->tmpdir);  
  ...
  if (ctrl_chr != BLK_START_AFIOBALLS)
    {
      ...
      if (ctrl_chr == BLK_START_EXTENDED_ATTRIBUTES)
        {
	  res = read_EXAT_files_from_tape(bkpinfo, &size, fname, &ctrl_chr, curr_xattr_list_fname, curr_acl_list_fname); 
	}
    }
  if (ctrl_chr != BLK_START_AFIOBALLS)
  ...
  for (res = read_header_block_from_stream (&size, fname, &ctrl_chr);
       ...
       res = read_header_block_from_stream (&size, fname, &ctrl_chr))
    {
      sprintf(curr_xattr_list_fname, XATTR_LIST_FNAME_RAW_SZ, bkpinfo->tmpdir, current_afioball_number);
      sprintf(curr_acl_list_fname, ACL_LIST_FNAME_RAW_SZ, bkpinfo->tmpdir, current_afioball_number);
      if (ctrl_chr == BLK_START_EXTENDED_ATTRIBUTES)
        {
  ...
  if (ctrl_chr != BLK_START_BIGGIEFILES)
    {
	  ...
	  res = read_EXAT_files_from_tape(bkpinfo, &size, orig_fname, &ctrl_chr, curr_xattr_list_fname, curr_acl_list_fname); 
	}
    }
  if (ctrl_chr != BLK_START_BIGGIEFILES)
    {
...
//skip
  ...
  if (!does_file_exist (tmp))
    {
      log_msg
	(2, "This is the last CD. I am therefore setting bkpinfo->verify_data to FALSE.");
...
//  fclose(g_tape_stream); <-- not needed; is handled by closein_tape()
  paranoid_system("rm -f /tmp/biggies.changed /tmp/changed.files.[0-9]* 2> /dev/null");
  ...
  sprintf (tmp,
	   ...
	   MONDO_LOGFILE, (bkpinfo->use_star)?"star":"afio", changed_files_fname);
