...
close_progress_form ()
{
  ...
  if (g_current_progress == -999)
    {
      log_msg (2, "Trying to close the progress form when it ain't open!");
      return;
    }
...
finish (int signal)
{
  ...
  run_program_and_log_output("rm -Rf /mondo.scratch.* /tmp.mondo.*", FALSE);
  ...
  if (!fin)
      ...
      for (i = 0; i < g_noof_log_lines; i++)
	{
	  for (err_log_lines[i][0] = '\0';
	    ...
	    {
	      (void) fgets (err_log_lines[i], MAX_NEWT_COMMENT_LEN, fin);
  ...
  if (g_text_mode)
  ...
  else
    {
      ...
      newtFormAddComponents (g_progressForm, g_percentline, g_timeline,
			     g_scale, g_blurb1, g_blurb3, g_blurb2, NULL);
...
popup_with_buttons (char *p, char *button1, char *button2)
{
  ...
  if (g_text_mode)
    {
      ...
      for(tmp[0]='\0'; strcmp(tmp, button1) && (strlen(button2)==0 || strcmp(tmp, button2)); )
        {
          printf("--> ");
          (void) fgets(tmp, MAX_NEWT_COMMENT_LEN, stdin);
        }
      if (!strcmp(tmp, button1)) { paranoid_free(tmp); paranoid_free(prompt); return(TRUE); }
      else { paranoid_free(tmp); paranoid_free(prompt); return(FALSE); }
    }
  ...
  if (b_res == b_1)
    {
paranoid_free(tmp); paranoid_free(prompt);       return (TRUE);
    }
  else
    {
paranoid_free(tmp); paranoid_free(prompt);       return (FALSE);
    }
}
  ...
  for (i = g_noof_log_lines - 1; i >= 0; i--)
    {
      ...
      newtDrawRootText (0, i + g_noof_rows - 1 - g_noof_log_lines, err_log_lines[i]);
    }
  newtRefresh ();
}
...
setup_newt_stuff ()
{
  ...
  err_log_lines = (char **) malloc (sizeof (char *) * g_noof_log_lines);
  if (!err_log_lines) {
...
//  log_it("update_eval_call_form called");
  ...
  if ((percentage < 3 && g_isoform_old_progress < 3)
	...
	{
	  newtScaleSet (g_isoform_scale, (unsigned long long) percentage);
...
//  log_msg(1, "'%s' '%s' '%s'", blurb1, blurb2, blurb3);
  ...
  if (g_text_mode)
      ...
      for (; i < 100; i += 5)
	{
      ...
      if (percentage>100) { log_msg(2, "percentage = %d", percentage); }
      sprintf (taskprogress + strlen (taskprogress),
	       "] %3d%c", percentage, '%');
      sprintf (taskprogress + strlen (taskprogress),
		" done; %2ld:%02ld to go",
...
which_backup_media_type (bool restoring)
{
  ...
  char *title_sz;
  char *minimsg_sz;
  static t_bkptype possible_bkptypes[] = { none,cdr,cdrw,dvd,tape,cdstream,udev,nfs,iso };
  static char *possible_responses[] = {"none", "cdr", "cdrw", "dvd", "tape", "cdstream", "udev", "nfs", "iso", NULL };
  char *outstr;
      ...
      for(backup_type = none; backup_type == none; )
        {
          ...
          for(i=0; possible_responses[i]; i++)
            {
              printf("%c%s", (i==0)?'\0':' ', possible_responses[i]);
            }
          printf(")\n--> ");
  ...
  myForm = newtForm (NULL, NULL, 0);
  newtFormAddComponents (myForm, b1, b5, b3, b7, b2, b4, b6, b8, NULL);
  b_res = newtRunForm (myForm);
  newtFormDestroy (myForm);
  newtPopWindow ();
...
int load_filelist_into_array(struct s_filelist *filelist, char *source_file)
{
  ...
  iamhere("entering");
  if (!(fin=fopen(source_file, "r"))) { log_OS_error(source_file); log_msg(2, "Can't open %s; therefore, cannot popup list", source_file); paranoid_free(tmp); return(1); }
          ...
          if (filelist->el[i].severity < filelist->el[i+1].severity || (filelist->el[i].severity==filelist->el[i+1].severity && strcmp(filelist->el[i].filename, filelist->el[i+1].filename)>0))
            {
              ...
              log_msg(2, "Swapping %s and %s", filelist->el[i].filename, filelist->el[i+1].filename);
              done = FALSE;
            }
        }
    }
...
char*filelist_entry_to_string(struct s_filelist_entry *flentry)
{
  ...
  malloc_string(tmp);
  assert(flentry!=NULL);
  if (flentry->severity==0) { strcpy(tmp, "0   "); }
  else if (flentry->severity==1) { strcpy(tmp, "low "); }
  else if (flentry->severity==2) { strcpy(tmp, "med "); }
  else { strcpy(tmp, "high"); }
  strcat(tmp, "  ");
...
void popup_changelist_from_file(char*source_file)
{
  ...
  if (lng < 1) 
    {
      log_msg (2, "No lines in file. Therefore, no popup list."); paranoid_free(reason);
      goto free_to_go;
     }
  ...
  if (load_filelist_into_array(filelist, source_file)) { log_msg (2, "Can't open %s; therefore, cannot popup list", source_file); paranoid_free(reason); return; }
  log_msg(2, "%d files loaded into filelist array", filelist->entries);
