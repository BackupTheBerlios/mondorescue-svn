...
cvsid                : $Id: mondo-cli.c,v 1.11 2004/06/21 20:20:36 hugo Exp $
 ***************************************************************************/
...
//static char cvsid[] = "$Id: mondo-cli.c,v 1.11 2004/06/21 20:20:36 hugo Exp $";
...
//      bkpinfo->media_size[0] = bkpinfo->media_size[MAX_NOOF_MEDIA];
      for(j=1; j<=MAX_NOOF_MEDIA; j++)
        {
          if (bkpinfo->media_size[j]<=0) { log_msg(1, "You gave media #%d an invalid size\n", j); return(-1); }
        }
      return(0);
}
...
/* compulsory */
  ...
  if (flag_set['I'])
      ...
      if (!strcmp(bkpinfo->include_paths, "/"))
	{
      ...
      strncpy (bkpinfo->include_paths+strlen(bkpinfo->include_paths), flag_val['I'], MAX_STR_LEN - strlen(bkpinfo->include_paths));
  ...
  if (g_kernel_version >= 2.6 && !flag_set['d'] && (flag_set['c'] || flag_set['w']))
    {
...
//	  log_msg(3, "flag_set['r'] = %i", flag_set['r'] );
	  if (flag_set['c']) { bkpinfo->cdrw_speed = atoi(flag_val['c']); }
	  else
	  if (flag_set['w']) { bkpinfo->cdrw_speed = atoi(flag_val['w']); }
	  else
	  if (flag_set['r']) { bkpinfo->cdrw_speed = 1; /*atoi(flag_val['r']);*/ }
  ...
  if (flag_set['t'] && !flag_set['d'])
    {
      ...
      flag_set['d'] = TRUE;
      sprintf(tmp, "You didn't specify a tape streamer device. I'm assuming %s", flag_val['d']);
      log_to_screen(tmp);
      percent = 0;
    }
...
*/
  ...
  if (flag_set['r']) { bkpinfo->backup_media_type = dvd; }
...
//      strncpy(psz, list_of_NFS_devices_and_mounts(), MAX_STR_LEN);
  ...
  if (flag_set['b'])
	...
	{ psz[strlen(psz)-1] = '\0'; itbs = atol(psz) * 1024L; }
      else
	{ itbs = atol(psz); }
...
//      bkpinfo->differential = atoi (flag_val['D']);
  ...
  if (flag_set['n'])
	...
	{
	  retval++;
	  sprintf (tmp, "Are you sure directory '%s' exists in remote dir '%s'?\nIf so, do you have rights to write to it?\n",
  ...
  if (!flag_set['d'] && (flag_set['c'] || flag_set['w'] || flag_set['C']))
    {
      if (g_kernel_version >= 2.6)
        {
	  if (popup_and_get_string("Device", "Please specify the device", bkpinfo->media_device, MAX_STR_LEN/4))
            {
	      retval ++;
  ...
  if (flag_set['T'])
    {
      ...
      sprintf(tmp, "ln -sf %s/.foo.dat %s/.bar.dat", flag_val['T'], flag_val['T']);
      if (run_program_and_log_output(tmp, 1))
        {
	  retval++;
...
#  endif
	...
	{
	  log_msg(1, "%c? WTF is %c? I need G, L, E or R.", bkpinfo->boot_loader, bkpinfo->boot_loader);
...
#undef BOOT_LOADER_CHARS
  ...
  if (!flag_set['o'] && !run_program_and_log_output("cat /etc/issue.net | grep -i suse | grep 64", TRUE))
    {
      bkpinfo->make_cd_use_lilo = TRUE;
      log_to_screen("Forcing you to use LILO. SuSE 9.0 (64-bit) has a broken mkfs.vfat binary.");
    }
  if (flag_set['o'])
    {
      bkpinfo->make_cd_use_lilo = TRUE;
  ...
  else
    {
	  ...
	  log_to_screen( "using LILO instead of SYSLINUX as the CD/floppy's boot loader.");
        }
                  ...
                  if (opt=='d')
                    {
                      if (strchr(flag_val[opt], '/') && flag_val[opt][0]!='/')
                        {
                          sprintf(tmp, "-%c flag --- must be absolute path --- '%s' isn't absolute", opt, flag_val[opt]);
                          log_to_screen(tmp);
                          bad_switches = TRUE;
                        }
		    }
...
void terminate_daemon(int sig)
{
  ...
  switch(sig)
	...
	case SIGKILL:
		sprintf(tmp, "SIGKILL");
		strcpy(tmp2, "I seriously have no clue how this signal even got to me. Something's wrong with your system.");
		break;
	case SIGTERM:
		sprintf(tmp, "SIGTERM");
	...
	case SIGSEGV:
		sprintf(tmp, "SIGSEGV");
		strcpy (tmp2, "Internal programming error. Please send a backtrace as well as your log.");
		break;
	case SIGPIPE:
		sprintf(tmp, "SIGPIPE");
    ...
    case SIGABRT:
	sprintf (tmp, "SIGABRT");
	sprintf (tmp2, "Abort - probably failed assertion. I'm sleeping for a few seconds so you can read the message.");
	break;
	default:
		sprintf(tmp, "(Unknown)");
    }
  ...
  kill_buffer();
  fatal_error("Mondoarchive is terminating in response to a signal from the OS");
