 ...
 */
void 
ask_about_these_imagedevs( char *infname, char *outfname )
{
  FILE *fin;
  FILE *fout;
  /************************************************************************
  ...
  for( fgets( incoming_ptr, MAX_STR_LEN, fin); 
       !feof( fin ); 
       fgets( incoming_ptr, MAX_STR_LEN, fin ) )
    {
	...
	{ 
      ...
      sprintf( question_ptr, 
	       "Should I restore the image of %s ?", 
	       incoming_ptr );
				  ...
				  char *output_mountlist_file)
{
  ...
  sprintf( command,
           "cp -f %s/%s %s", // %s/%s becomes {mountpt}/tmp/m*ndo-restore.cfg
	   mountpt, g_mondo_cfg_file, 
	   output_cfg_file );
  run_program_and_log_output(command, FALSE);
  ...
  if (does_file_exist( orig_fname ))
    {
      sprintf( command, 
	   "cp -f %s %s",
	   orig_fname, output_mountlist_file );
      run_program_and_log_output( command, FALSE );
    }
  sprintf( command, "umount %s", mountpt );
  run_program_and_log_output( command, FALSE );
 ...
 */
bool
is_file_in_list(char *f, char *list_fname, char *preamble)
{
...
/* End patch */
  ...
  sprintf(tmp,"%s #%d has been mounted via loopback mount", media_descriptor_string(bkpinfo->backup_media_type), i);
  log_msg(1,tmp);
  if (i<0)
    { popup_and_OK("Cannot find ISO images in the directory you specified."); retval=1; }
  log_msg(2,"%ld: bkpinfo->isodir is now %s", __LINE__, bkpinfo->isodir);
...
kill_petris(void)
{
  char *command;
  malloc_string(command);
  sprintf( command, "kill `ps wax 2> /dev/null | grep petris 2> /dev/null | grep -v grep | cut -d' ' -f2` 2> /dev/null");
 ...
 */
int
modify_rclocal_one_time( char *path )
{
  /** malloc **/
  char *rclocal_fname;
  char *newfile_fname;
...
int mount_all_devices(struct mountlist_itself *p_external_copy_of_mountlist, bool writeable)
{
  ...
  for( lino = 0; lino < mountlist->entries; lino++ )
      ...
      else if ( is_this_device_mounted( mountlist->el[lino].device ) )
	{
      ...
      else if ( strcmp( mountlist->el[lino].mountpoint, "none" ) && strcmp( mountlist->el[lino].mountpoint, "lvm" ) && strcmp( mountlist->el[lino].mountpoint,"raid" ) && strcmp(mountlist->el[lino].mountpoint, "image" ) )
	{
	  sprintf( tmp, "Mounting %s",mountlist->el[lino].device );
	  update_progress_form( tmp );
	  strcpy(format, mountlist->el[lino].format);
	  if (!strcmp(format, "ext3")) { strcpy(format, "ext2"); }
...
int mount_cdrom(struct s_bkpinfo *bkpinfo)
{
...
#ifdef __FreeBSD__
      sprintf (mount_cmd, "/mnt/isodir/%s/%s/%d.iso", bkpinfo->isodir, bkpinfo->nfs_remote_dir, g_current_media_number);
      mddev = make_vn (mount_cmd);
      sprintf (mount_cmd, "mount_cd9660 -r %s "MNT_CDROM, mddev);
#else
      sprintf(mount_cmd, "mount %s/%s/%d.iso -t iso9660 -o loop,ro %s", bkpinfo->isodir, bkpinfo->nfs_remote_dir, g_current_media_number, MNT_CDROM);
#endif
...
#else
      sprintf(mount_cmd, "mount %s/%d.iso -t iso9660 -o loop,ro %s", bkpinfo->isodir, g_current_media_number, MNT_CDROM);
#endif
    }
  else if (strstr(bkpinfo->media_device, "/dev/"))

#ifdef __FreeBSD__
    { sprintf(mount_cmd, "mount_cd9660 -r %s %s", bkpinfo->media_device, MNT_CDROM); }
#else
    { sprintf(mount_cmd, "mount %s -t iso9660 -o ro %s", bkpinfo->media_device, MNT_CDROM); }
#endif
  ...
  if (res) { log_msg(2,"Failed, despite %d attempts", i); }
  else { log_msg(2,"Mounted CD-ROM drive OK"); }
  paranoid_free(mount_cmd);
  return(res);
}
 ...
 */
int
mount_device( char *device, char *mpt, char *format, bool writeable)
{
  int res = 0;
  ...
  if ( !strcmp( mountpoint,"swap" ) )
  ...
  else
    {
      ...
      run_program_and_log_output( command, FALSE );
      sprintf( command, "mount -t %s %s %s %s 2>> %s", format, device, additional_parameters, mountdir, MONDO_LOGFILE );
  ...
  if ( res && (strstr(command, "xattr") || strstr(command, "acl")))
    {
      ...
      sprintf( command, "mount -t %s %s %s 2>> %s", format, device, mountdir, MONDO_LOGFILE );
      res = run_program_and_log_output( command, TRUE );
    }
  if ( res )
    {
      ...
      else
	{
	  ...
	  if (res == 0 )
	    {
	      log_msg(1, "That's OK. I called mount w/o a filesystem type and it worked fine in the end." );
	    }
	  else
	    {
	      log_to_screen( tmp );
...
{
  run_program_and_log_output( "touch " MNT_RESTORING "/var/log/pacct", FALSE );
  run_program_and_log_output( "touch " MNT_RESTORING "/var/account/pacct", FALSE );
  if (run_program_and_log_output( "ls " MNT_RESTORING" /tmp", FALSE ) )
     {
      run_program_and_log_output( "chmod 1777 " MNT_RESTORING "/tmp", FALSE );
    }
  run_program_and_log_output( "mkdir -p " MNT_RESTORING "/var/run/console", FALSE );
  run_program_and_log_output( "chmod 777 " MNT_RESTORING "/dev/null", FALSE );
  run_program_and_log_output( "cd " MNT_RESTORING "; for i in `ls home/`; do echo \"Moving $i's spurious files to $i/.disabled\"; mkdir $i/.disabled ; mv -f $i/.DCOP* $i/.MCOP* $i/.*authority $i/.kde/tmp* $i/.kde/socket* $i/.disabled/ ; done", TRUE);
  run_program_and_log_output( "rm -f " MNT_RESTORING "/var/run/*.pid", TRUE);
  run_program_and_log_output( "rm -f " MNT_RESTORING "/var/lock/subsys/*", TRUE);
}
/**************************************************************************
 ...
 */
int
read_cfg_file_into_bkpinfo( char* cfgf, struct s_bkpinfo *bkpinfo)
{
  /** add mallocs **/
  char *value;
  char *tmp;
...
//          bkpinfo->media_size[0] = -1;
  ...
  if (0 == read_cfg_var(cfg_file, "internal-tape-block-size", value))
    {
      ...
      log_msg(1, "Internal tape block size has been custom-set to %ld", bkpinfo->internal_tape_block_size);
    }
  else
    {
      bkpinfo->internal_tape_block_size = DEFAULT_INTERNAL_TAPE_BLOCK_SIZE;
      log_msg(1, "Internal tape block size = default (%ld)", DEFAULT_INTERNAL_TAPE_BLOCK_SIZE);
    }
  ...
  if (tmp[0] || strstr(call_program_and_get_last_line_of_output("cat /proc/cmdline"), "donteject"))
    { bkpinfo->please_dont_eject = TRUE; log_msg(2, "Ok, I shan't eject when restoring! Groovy."); }
      ...
      if (!cfgf)
        {
          ...
          log_msg(2, "nfs_remote_dir remains %s", bkpinfo->nfs_remote_dir);
	  log_msg(2, "...cos it wouldn't make sense to abandon the values that GOT ME to this config file in the first place");
	}
      else
        {
          read_cfg_var(g_mondo_cfg_file, "nfs-server-mount", bkpinfo->nfs_mount);
          read_cfg_var(g_mondo_cfg_file, "nfs-server-path", bkpinfo->nfs_remote_dir);
  ...
  */
      ...
      if (!bkpinfo->disaster_recovery)
	    ...
	    if (strcmp(old_isodir, bkpinfo->isodir)) {
	      log_it("user nominated isodir differs from archive, keeping user's choice: %s %s\n", old_isodir, bkpinfo->isodir);
		strcpy (bkpinfo->isodir, old_isodir);
	    }
        }
      read_cfg_var(g_mondo_cfg_file, "iso-dev", g_isodir_device);
	  ...
	  if (is_this_device_mounted(g_isodir_device))
		...
		/* Find out where it's mounted */
		 sprintf(command, "mount | grep -w %s | tail -n1 | cut -d' ' -f3", g_isodir_device);
		...
		sprintf(iso_mnt, "%s", call_program_and_get_last_line_of_output(command));
		  ...
		  if (run_program_and_log_output(tmp, 3))
			...
			if ( mount_cdrom( bkpinfo ) )
			  ...
			  {
			    log_msg(1, "You backed up to disk, then burned some CDs. Naughty monkey!" );
			  }
		      }
	      }
...
s_node *process_filelist_and_biggielist( struct s_bkpinfo *bkpinfo )
{
  ...
  if ( does_file_exist( g_filelist_full ) && does_file_exist( g_biggielist_txt ) )
    {
      log_msg(1, "%s exists", g_filelist_full);
      log_msg(1, "%s exists", g_biggielist_txt);
      log_msg(2, "Filelist and biggielist already recovered from media. Yay!" );
    }
  else
    {
...
//	  popup_and_OK("Press ENTER to continue");
          ...
          if ( !does_file_exist( FILELIST_FULL_STUB ) )
            {
	      fatal_error( "all.tar.gz did not include tmp/filelist.full.gz");
	    }
        }
      sprintf( command, "cp -f %s %s", MONDO_CFG_FILE_STUB, g_mondo_cfg_file );
      run_program_and_log_output( command, FALSE );

      sprintf(command, "cp -f %s/%s %s", bkpinfo->tmpdir, BIGGIELIST_TXT_STUB, g_biggielist_txt);
      log_msg(1, "command = %s", command);
      paranoid_system(command);
      sprintf(command, "ln -sf %s/%s %s", bkpinfo->tmpdir, FILELIST_FULL_STUB, g_filelist_full);
      log_msg(1, "command = %s", command);
      paranoid_system(command);
    }

  if (am_I_in_disaster_recovery_mode() && ask_me_yes_or_no("Do you want to retrieve the mountlist as well?"))
    {
//      sprintf(command, "cp -f tmp/mountlist.txt /tmp");
      sprintf(command, "ln -sf %s/%s /tmp", MOUNTLIST_FNAME_STUB, bkpinfo->tmpdir);
      paranoid_system(command);
    }
...
//  popup_and_OK("Wonderful.");
  ...
  switch( pid )
      ...
      if (!does_file_exist(g_biggielist_txt))
	{
      ...
      sprintf(command,"cat %s | grep  -x \"/dev/.*\" > %s", g_biggielist_txt, g_filelist_imagedevs);
      paranoid_system(command);
      exit( 0 );
      break;
  ...
  if (g_text_mode)
    {
      ...
      if (strlen(tmp)==0) { res=1; } else { res=0; }
    }
  else
    {
  ...
  if ( length_of_file( g_imagedevs_restthese ) > 2 )
    { add_list_of_files_to_filelist(filelist, g_imagedevs_restthese, TRUE); }
 ...
 */
int
run_boot_loader(bool offer_to_hack_scripts)
{
  int res;
  int retval = 0;
 ...
 */
int
run_grub(bool offer_to_run_stabgrub, char*bd)
{
  /** malloc **/
  char *command;
  char *boot_device;
  ...
  if (!run_program_and_log_output("which grub-MR", FALSE))
    ...
    {
      sprintf(command, "chroot " MNT_RESTORING " grub-install %s",boot_device);
  ...
  if (offer_to_run_stabgrub && ask_me_yes_or_no("Did you change the mountlist?"))
      ...
      for(done = FALSE; !done; )
	{
	  ...
	  if (res)
	    {
	      popup_and_OK("GRUB installation failed. Please install manually using 'grub-install' or similar command. You are now chroot()'ed to your restored system. Please type 'exit' when you are done.");
	      newtSuspend();
  ...
  if (res)
    {
      ...
      log_msg(1,"If you're really stuck, please e-mail the mailing list.");
    }
  else
    {
      mvaddstr_and_log_it(g_currentY++, 74, "Done.");
 ...
 */
int 
run_elilo(bool offer_to_run_stabelilo)
{
  /** malloc **/
  char *command;
  char *tmp;
  ...
  if (offer_to_run_stabelilo && ask_me_yes_or_no("Did you change the mountlist?"))
    ...
    /* interactive mode */
    {
      ...
      if (res)
        {
          popup_and_OK("You will now edit fstab and elilo.conf, to make sure they match your new mountlist.");
          for(done = FALSE; !done;)
            {
              if (!g_text_mode) { newtSuspend(); }
	      sprintf(tmp, "%s " MNT_RESTORING "/etc/fstab", editor);
	      paranoid_system(tmp);
	      sprintf(tmp, "%s " MNT_RESTORING "/etc/elilo.conf", editor);
	      paranoid_system(tmp);
              if (!g_text_mode) { newtResume(); }
//              newtCls();
 ...
 */
int 
run_lilo(bool offer_to_run_stablilo)
{
  /** malloc **/
  char *command;
  char *tmp;
  ...
  if (!run_program_and_log_output("grep \"boot.*=.*/dev/md\" " MNT_RESTORING "/etc/lilo.conf", 1))
        { run_lilo_M = TRUE; }
...
//              newtCls();
  ...
  if (run_lilo_M)
    {
      run_program_and_log_output("chroot " MNT_RESTORING " lilo -M /dev/hda", 3);
      run_program_and_log_output("chroot " MNT_RESTORING " lilo -M /dev/sda", 3);
    }
  paranoid_free(command);
  paranoid_free(tmp);
  paranoid_free(editor);
 ...
 */
int 
run_raw_mbr(bool offer_to_hack_scripts, char *bd)
{
  /** malloc **/
  char *command;
  char *boot_device;
 ...
 */
void
setup_MR_global_filenames(struct s_bkpinfo *bkpinfo)
{
  char *temppath;
...
//  sprintf(g_imagedevs_pot, "%s/tmp/imagedevs.pot", temppath);
  sprintf(g_imagedevs_restthese, "%s/tmp/imagedevs.restore-these", temppath);
  if (bkpinfo->disaster_recovery)
    {
 ...
 */
void 
streamline_changes_file(char *output_file, char *input_file)
{
  FILE *fin;
  FILE *fout;
  /** malloc **/
  ...
  else
    {
      if (ask_me_yes_or_no("Are you or your company willing to consider contributing to Mondo in some way?"))
      ...
      else
        {
          ...
          log_msg(1, "To your credit, you were honest: you said no, you wouldn't be contributing");
          log_msg(1, "to this project, ever. However, that makes you a freeloader. I bet you're");
          log_msg(1, "the sort of person who likes to sneak into movie theatres...");
...
twenty_seconds_til_yikes()
{
  ...
  malloc_string(tmp);
  if (does_file_exist("/tmp/NOPAUSE")) { return; }
 ...
 */
void
termination_in_progress(int sig)
{
...
int unmount_all_devices(struct mountlist_itself *p_external_copy_of_mountlist)
{
  ...
  open_progress_form("Unmounting devices", 
		     ...
		     mountlist->entries);
  chdir( "/" );
  for(i = 0; i < 10 && run_program_and_log_output("ps wax | grep buffer | grep -v \"grep buffer\"", TRUE) == 0; i++)
    {
      sleep( 1 );
  ...
  if (run_program_and_log_output( "cp -f /tmp/mondo-restore.log " MNT_RESTORING "/tmp/", FALSE ) )
    { 
      log_msg(1, "Error. Failed to copy log to PC's /tmp dir. (Mounted read-only?)");
    }
  if (run_program_and_log_output( "cp -f /tmp/mondo-restore.log " MNT_RESTORING "/root/", FALSE ) )
    {
      log_msg(1, "Error. Failed to copy log to PC's /root dir. (Mounted read-only?)");
    }
  ...
  for( lino = mountlist->entries-1; lino >= 0; lino-- )
    {
      ...
      if (is_this_device_mounted(mountlist->el[lino].device))
		...
		if (!strcmp (mountlist->el[lino].mountpoint, "/1"))
			...
			log_msg (3, "Well, I know a certain kitty-kitty who'll be sleeping with Mommy tonight...");
		    }
		else
		    { 
			sprintf(command,"umount " MNT_RESTORING "%s",mountlist->el[lino].mountpoint);
		    }
	    }
  ...
  // tar -zxvf-
	   sprintf( command,
		"dd if=%s bs=%ld count=%ld 2> /dev/null | tar -zx %s %s %s %s %s",
		   ...
		   BIGGIELIST_TXT_STUB, FILELIST_FULL_STUB,
		   "tmp/i-want-my-lvm" );
	   log_msg (2, "command = '%s'", command);
           res = run_program_and_log_output( command, -1 );
	   if (res!=0 && does_file_exist(MONDO_CFG_FILE_STUB)) { res=0; }
  paranoid_free(command);
  return(res);
}
 ...
 */
int 
get_cfg_file_from_archive( struct s_bkpinfo *bkpinfo )
{
   int retval = 0;
...
//   unlink( "tmp/mondo-restore.cfg" ); // superfluous, surely?
   ...
   sprintf( tmp, "(sleep 15; kill `ps ax | grep \"%s\" | cut -d' ' -f1` 2> /dev/null) &", command);
   log_msg(1, "tmp = '%s'", tmp);
   system(tmp);
   res = run_program_and_log_output( command, FALSE );
   if (res) {
   ...
   if (res) {
	try_plan_B=TRUE;
        ...
        try_plan_B=TRUE;
        log_msg(2, "Mounted floppy OK but I don't trust it because the archives might contain more up-to-date config file than the floppy does.");
// NB: If busybox does not support 'mount -o loop' then Plan A WILL NOT WORK.
 	...
 	if ( !does_file_exist( ramdisk_fname ) )
 	  { 
 	    log_msg(2, "Cannot find ramdisk file on mountpoint. Are you sure that's a boot disk in the drive?" );
 	  }
 	if ( extract_config_file_from_ramdisk( bkpinfo, ramdisk_fname, cfg_file, mountlist_file ) )
 	  {
 	    log_msg(2, "Warning - failed to extract config file from ramdisk. I think this boot disk is mangled." );
 	  }
 	sprintf( command, "umount %s", mountpt );
        run_program_and_log_output(command, 5);
...
log_msg(2, "gcffa --- managed to mount CD; so, no need for Plan B" );
	   ...
	   if (strlen(bkpinfo->media_device) ==0 )
        ...
        {
log_msg(2, "gcffa --- looking at mounted CD for mindi-boot.2880.img" );
          sprintf( command, "mount "MNT_CDROM"/images/mindi-boot.2880.img -o loop %s", mountpt );
          sprintf( mounted_cfgf_path, "%s/%s", mountpt, cfg_file );
          if (!does_file_exist(mounted_cfgf_path))
            {
              log_msg(2,"gcffa --- Plan C, a.k.a. untarring some file from all.tar.gz" );
              sprintf( command,
		       "tar -zxvf "MNT_CDROM"/images/all.tar.gz %s %s %s %s %s",
		        MOUNTLIST_FNAME_STUB, MONDO_CFG_FILE_STUB, BIGGIELIST_TXT_STUB, FILELIST_FULL_STUB, "tmp/i-want-my-lvm"
		       ); // add -b TAPE_BLOCK_SIZE if you _really_ think it's necessary
  ...
  if ( does_file_exist( MONDO_CFG_FILE_STUB) )
    {
      ...
      sprintf(tmp, "%s/%s", call_program_and_get_last_line_of_output("pwd"), MONDO_CFG_FILE_STUB);
      sprintf( command, "cp -f %s %s", tmp, cfg_file);
      iamhere(command);
      if (strcmp(tmp, cfg_file) && run_program_and_log_output( command, 1 ))
        { log_msg(1,"... but an error occurred when I tried to move it to %s", cfg_file); }
      else
        { log_msg(1,"... and I moved it successfully to %s", cfg_file); }
      sprintf(command, "cp -f %s/%s %s", call_program_and_get_last_line_of_output("pwd"), MOUNTLIST_FNAME_STUB, mountlist_file);
      iamhere(command);
      if (strcmp(tmp, cfg_file) && run_program_and_log_output( command, 1 ))
	  ...
	  sprintf(command, "cp -f %s %s", mountlist_file, g_mountlist_fname);
...
/* start SAH */
    ...
    {
      sprintf(command, "cp -f %s %s/%s", MOUNTLIST_FNAME_STUB, bkpinfo->tmpdir, MOUNTLIST_FNAME_STUB);
      run_program_and_log_output(command,FALSE);
    }
      ...
      for(unfinished_mdstat_devices=i=0; i<mdstat->entries; i++)
        {
		  ...
		  sleep(2);
		  if (read_mdstat(mdstat, mdstat_file)) {break;}
		}
	      close_evalcall_form();
	    }
        }
