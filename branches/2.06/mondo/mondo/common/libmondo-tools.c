...
06/28
- abort if RAID partitions found but /etc/raidtab not found

06/25
- don't insist on ms-sys; recommend it
- don't fatal_error if can't open log file

06/14
- call mindi -V before calling mindi --makemountlist

04/09
- lots of malloc'd char*'s instead of char[]'s in post_param_config'n

04/02
- better sanity-checking, re: /etc/modules.conf

02/15
- abort if Windows dir but no ms-sys or parted

02/08
- missing ramdisk/initrd support is nonfatal now (but stupid)

01/16
- added g_kernel_version and get_kernel_version()

01/08/2004
- fixed /boot mounter thingy

11/14/2003
- if <50MB free in /root, softlink /root/images/mindi to
  /home/root/images/mindi as a workaround

10/23
- streaming backups now generate 4MB sets; CDs, 8MB sets

10/21
- better at finding your /boot in order to mount it, if nec.
- link /etc/modules.conf to /etc/conf.modules if former
  is missing but latter is not
- changed "/mnt/cdrom" to MNT_CDROM
- changed "/mnt/floppy" to MNT_FLOPPY

10/13
- added stop_magicdev_if_necessary()
- added restart_magicdev_if_necessary()

10/02
- don't eject if bkpinfo->please_dont_eject is TRUE

09/28
- fixed segfault in mount_..._necessary

09/26
- ask for cmp (or create workaround if possible)

09/25
- add DVD support

09/24
- don't overwrite existing media-dev value in bkpinfo IF it's
  a tape device AND it's already populated by /dev/{something}
  ...This helps w/ weird tape drives
...
09/18
- added stop_autofs_if_necessary() and restart_autofs_if_necessary()

09/13
- added log_debug_msg()
- turned log_it() into a macro

09/10
- reduced PPCFG_RAMDISK to 150m (was 200m)
- max set size is now 8MB (was 6)
- added 'buffer' to requisite tools

08/29
- typical set size is now 5MB
   
07/22
- increased PPCFG* to 200m

05/07
- changed grep -m1 to grep | head -n1 for Debian users
- pre_param_...() also mkdir -p /mnt/cdrom, just in case

05/05
- cleaner logging of call to dd if=<tape> of=/dev/null
...
05/02
- log_it() --- "Message repeated N times" (if it was)

04/24
- added lots of assert()'s and log_OS_error()'s
- reset_bkpinfop() --- reset all media_size[] array
- increased tmpfs ramdisk to size=100m

04/22
- post_param_configuration() --- store iso-dev and isodir

04/05
- re-enable tmpfs ramdisk/mountpt _but_ make sure size=80m
...
int read_cfg_var(char *config_file, char *label, char *value)
{
	...
	if (!does_file_exist(config_file)) {
		...
		sprintf(command, "grep '%s .*' %s| cut -d' ' -f2,3,4,5",
				label, config_file);
