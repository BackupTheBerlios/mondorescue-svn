				   ...
				   struct mountlist_itself *unallocated_raid_partitions)
{
  ...
  /** prototypes *********************************************************/
	...
	sprintf(tmp, "%-26s %s", _("Device"), _("Size"));
...
choose_raid_level(struct OSSWAP (raid_device_record, vinum_plex) * raidrec)
{
	...
	sprintf(prompt, _("Please enter the RAID level you want. %s"),
			personalities);
					  ...
					  int currline)
{
	...
	sprintf(tmp, _("Delete %s from RAID device %s - are you sure?"),
					   ...
					   void *keylist[])
{
	...
	pos =
		which_raid_device_is_using_this_partition(raidlist,
												  ...
												  device);
	if (pos >= 0) {
		sprintf(tmp, _("Cannot delete %s: it is in use by RAID device %s"),
				mountlist->el[currline].device,
				raidlist->el[pos].OSSWAP(raid_device, volname));
		popup_and_OK(tmp);
		return;
	}
	sprintf(tmp, _("Delete %s - are you sure?"),
			mountlist->el[currline].device);
	if (!ask_me_yes_or_no(tmp)) {
		return;
	}
					  ...
					  struct raidlist_itself *raidlist, char *device)
{
	...
	sprintf(tmp, _("Do you want me to delete %s's partitions, too?"), device);
...
void delete_varslist_entry(struct raid_device_record *raidrec, int lino)
{
	...
	sprintf(tmp, _("Delete %s - are you sure?"), av->el[lino].label);
	if (ask_me_yes_or_no(tmp)) {
		if (!strcmp(av->el[lino].label, "persistent-superblock")
			|| !strcmp(av->el[lino].label, "chunk-size")) {
			sprintf(tmp, _("%s must not be deleted. It would be bad."),
					av->el[lino].label);
			popup_and_OK(tmp);
		} else {
