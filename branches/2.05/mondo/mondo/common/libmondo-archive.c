...
static int semaphore_p(void) // changes status to 'P' (waiting)
{
  ...
  if (semop(g_sem_id, &sem_b, 1) == -1)
    { log_msg(3, "semaphore_p failed"); return(0); }
  return(1);
}
...
static int semaphore_v(void) // changes status to 'V' (free)
{
  ...
  sem_b.sem_flg = SEM_UNDO;
  if (semop(g_sem_id, &sem_b, 1) == -1)
    { log_msg(3, "semaphore_v failed"); return(0); }
  return(1);
}
...
int
archive_this_fileset_with_star (struct s_bkpinfo *bkpinfo, char *filelist, char *fname,
	      int setno)
{
  int retval = 0;
  unsigned int res = 0;
  ...
  sprintf(command, "star H=star list=%s -c " STAR_ACL_SZ " file=%s", filelist, fname);
  if (bkpinfo->use_lzo) { fatal_error("Can't use lzop"); }
  ...
  for(res=99,tries=0; tries<3 && res!=0; tries++)
    {
      ...
      if (bkpinfo->use_star && (res==254 || res==65024) && strstr(tmp, "star: Processed all possible files") && tries>0)
	{
	  ...
	  if (p)
	    ...
	    {
              log_msg (3, "Attempt #%d failed. Pausing 3 seconds and retrying...", tries+1);
	      sleep(3);
	    }
        }
    }
  retval += res;
  if (retval) { log_msg (3, "Failed to write set %d", setno); }
  else if (tries>1) { log_msg( 3,"Succeeded in writing set %d, on try #%d", setno, tries); }
	      ...
	      int setno)
{
  ...
  if (bkpinfo->compression_level > 0 && bkpinfo->use_star)
    {
      return(archive_this_fileset_with_star(bkpinfo, filelist, fname, setno));
    }
  ...
  if (bkpinfo->compression_level > 0)
    {
      ...
      sprintf (zipparams, "-Z -P %s -G %d -T 3k", bkpinfo->zip_exe, bkpinfo->compression_level);
...
//  make_hole_for_file(fname);
  ...
  if (!does_file_exist(bkpinfo->tmpdir)) { log_OS_error("tmpdir not found"); fatal_error("tmpdir not found"); }
  if (!does_file_exist(bkpinfo->scratchdir)) { log_OS_error("scratchdir not found"); fatal_error("scratchdir not found"); }
  sprintf (command, "rm -f %s %s. %s.gz %s.%s", fname, fname, fname, fname,
	   bkpinfo->zip_suffix);
  paranoid_system (command);

  sprintf (command, "afio -o -b %ld -M 16m %s %s < %s 2>> %s", TAPE_BLOCK_SIZE, zipparams, fname, filelist, MONDO_LOGFILE);
  ...
  if (g_tmpfs_mountpt[0] != '\0') {
      ...
      if (i>0)
	  {
	      if (free_ramdisk_space > i)
		  {
		      ...
		      log_msg (2, "min(free_ramdisk_space) is now %d", free_ramdisk_space);
		      if (free_ramdisk_space < 10)
			  {
			      fatal_error ("Please increase PPCFG_RAMDISK_SIZE in my-stuff.h to increase size of ramdisk ");
			  }
		  }
	  }
  }
  ...
  set_g_cdrom_and_g_dvd_to_bkpinfo_value(bkpinfo);
   malloc_string(tmp);
      if (bkpinfo->backup_media_type == dvd)
        {
#ifdef DVDRWFORMAT
	  if (!find_home_of_exe("dvd+rw-format")) { fatal_error("Cannot find dvd+rw-format. Please install it or fix your PATH."); }
#endif
	  if (!find_home_of_exe("growisofs")) { fatal_error("Cannot find growisofs. Please install it or fix your PATH."); }
	}

      if ((res=prepare_filelist (bkpinfo))) /* generate scratchdir/filelist.full */
        { fatal_error ("Failed to generate filelist catalog"); }
      if (call_filelist_chopper (bkpinfo)) { fatal_error("Failed to run filelist chopper"); }
...
*/
      ...
      sprintf(tmp, "cp -f %s/archives/*list*.gz %s", bkpinfo->scratchdir, bkpinfo->tmpdir);
      if (run_program_and_log_output(tmp, 2))
...
malloc_string ( bootdev );
  ...
  if (!does_file_exist(tmp))
    {
      ...
      if (!does_file_exist(tmp))
        { fatal_error("Cannot find filelist.full, so I cannot count its lines"); }
    }
...
#endif
      bkpinfo->boot_loader != 'L' && bkpinfo->boot_loader != 'G' && bkpinfo->boot_loader != 'R' && !bkpinfo->nonbootable_backup)
    {
      fatal_error
  ...
  if (bkpinfo->boot_loader == 'L')
	...
	{
	  run_program_and_log_output("ln -sf /boot/grub/grub.conf /etc/grub.conf", 5);
	}
      /* Detect Debian's grub config file */
      else if (!does_file_exist("/etc/grub.conf") && does_file_exist("/boot/grub/menu.lst"))
        {
          run_program_and_log_output("ln -s /boot/grub/menu.lst /etc/grub.conf", 5);
        }
  ...
  else if (bkpinfo->boot_loader == 'E')
    {
      ...
      if (!does_file_exist("/etc/elilo.conf") && does_file_exist("/boot/efi/efi/redhat/elilo.conf"))
	{
	  run_program_and_log_output("ln -sf /boot/efi/efi/redhat/elilo.conf /etc/elilo.conf", 5);
	}
      if (!does_file_exist("/etc/elilo.conf"))
	{ fatal_error("The de facto mondo standard location for your boot loader's config file is /etc/elilo.conf but I cannot find it there. What is wrong with your Linux distribution? Try finding it under /boot/efi and do 'ln -s /boot/efi/..../elilo.conf /etc/elilo.conf'"); }
    }
  ...
  if (write_one_liner_data_file (tmp, bkpinfo->boot_device)) { log_msg(1, "%ld: Unable to write one-liner boot device", __LINE__); }
  switch(bkpinfo->backup_media_type)
    {
      ...
      case dvd: strcpy(value, "dvd"); break;
      default: fatal_error("Unknown backup_media_type");
    }
  sprintf (tmp, "%s/BACKUP-MEDIA-TYPE", bkpinfo->tmpdir);
  if (write_one_liner_data_file (tmp, value)) { res++; log_msg(1, "%ld: Unable to write one-liner backup-media-type", __LINE__); }
  log_to_screen(bkpinfo->tmpdir);
  sprintf (tmp, "%s/BOOTLOADER.NAME", bkpinfo->tmpdir);
  if (write_one_liner_data_file (tmp, bootldr_str)) { res++; log_msg(1, "%ld: Unable to write one-liner bootloader.name", __LINE__); }
...
/* add nfs stuff here? */
  ...
  if (system (command)) { res++; log_OS_error("Unable to make images directory"); }
  sprintf (command, "mkdir -p %s%s", bkpinfo->scratchdir, MNT_FLOPPY);
  if (system (command)) { res++; log_OS_error("Unable to make mnt floppy directory"); }
...
//  popup_and_OK("Pausing");
  ...
  if (!res)
    {
      ...
      sprintf (command, "cp -f %s/images/mindi.iso /root/images/mindi/mondorescue.iso",
    	   bkpinfo->scratchdir);
      log_msg (2, command);
      run_program_and_log_output (command, FALSE);
      ...
      sprintf(command, "cp -f %s/mindi-*oot*.img %s/images", bkpinfo->tmpdir, bkpinfo->scratchdir);
  ...
  paranoid_free(tmp); paranoid_free(use_lzo_sz); paranoid_free(scratchdir);
  paranoid_free(use_comp_sz); paranoid_free(bootldr_str); paranoid_free(tape_device);
  paranoid_free(last_filelist_number); paranoid_free(broken_bios_sz); paranoid_free(cd_recovery_sz);
  paranoid_free(tape_size_sz); paranoid_free(devs_to_exclude); paranoid_free(use_lilo_sz); paranoid_free(value);
  paranoid_free(bootdev); paranoid_free(command); paranoid_free(use_star_sz);
  return (res);
}
...
void *create_afio_files_in_background(void*inbuf)
{
  ...
  sprintf(archiving_filelist_fname, FILELIST_FNAME_RAW_SZ, bkpinfo->tmpdir, 0L);
  for (archiving_set_no = 0; does_file_exist (archiving_filelist_fname);
	...
	{ fatal_error("Maximum number of filesets exceeded. Adjust MAX_NOOF_SETS_HERE, please."); }
      if (!semaphore_p()) { log_msg(3, "P sem failed (pid=%d)", (int)getpid()); fatal_error("Cannot get semaphore P"); }
      ...
      if (does_file_exist(tmp))
	{
      ...
      sprintf (curr_xattr_list_fname, XATTR_LIST_FNAME_RAW_SZ, bkpinfo->tmpdir, archiving_set_no);
      sprintf (curr_acl_list_fname, ACL_LIST_FNAME_RAW_SZ, bkpinfo->tmpdir, archiving_set_no);
...
hexdump | tr -s ' ' '0' | head -n1"));
  ...
  if (write_one_liner_data_file (tmpfile, g_serial_string)) { log_msg(1, "%ld: Failed to write serial string", __LINE__); }
...
make_afioballs_and_images (struct s_bkpinfo *bkpinfo)
{
  ...
  transfer_block = malloc(sizeof(struct s_bkpinfo)+BKPINFO_LOC_OFFSET+64);
  memset((void*)transfer_block, 0, sizeof(struct s_bkpinfo)+BKPINFO_LOC_OFFSET+64);
  ...
  *p_next_set_to_archive = 0;
  ...
  while(!done_storing)
    {
      if (g_exiting) { fatal_error("Execution run aborted (main loop)"); }
      if (*p_archival_threads_running == 0 && *p_last_set_archived == storing_set_no-1)
	{
	  log_msg(2, "No archival threads are running. The last stored set was %d and I'm looking for %d. Take off your make-up; the party's over... :-)", *p_last_set_archived, storing_set_no);
	  done_storing = TRUE;
	}
      else if (!get_bit_N_of_array(p_list_of_fileset_flags, storing_set_no))
	{
	  misc_counter_that_is_not_important = (misc_counter_that_is_not_important+1) % 5;
	...
	// store set N
	{
	  ...
	  while(!does_file_exist(storing_filelist_fname) || !does_file_exist(storing_afioball_fname))
	    {
	      log_msg (2, "Warning - either %s or %s doesn't exist yet. I'll pause 5 secs.", storing_filelist_fname, storing_afioball_fname);
	      sleep(5);
	    }
	  ...
	  if (IS_THIS_A_STREAMING_BACKUP(bkpinfo->backup_media_type))
	    {
              register_in_tape_catalog(fileset, storing_set_no, -1, storing_afioball_fname);
	      maintain_collection_of_recent_archives(bkpinfo->tmpdir, storing_afioball_fname);
	      iamhere("Writing EXAT files");
              res += write_EXAT_files_to_tape(bkpinfo, curr_xattr_list_fname, curr_acl_list_fname);
// archives themselves
	      res += move_files_to_stream (bkpinfo, storing_afioball_fname, NULL);
	    }
	  else
	    {
	      res = move_files_to_cd (bkpinfo, storing_filelist_fname, curr_xattr_list_fname, curr_acl_list_fname, storing_afioball_fname, NULL);
	    }
	  retval += res;
	  g_current_progress++;
	  update_progress_form (media_usage_comment);
  ...
  if (bkpinfo->call_make_iso[0] != '\0')
    {
      ...
      if (retval)
         {
	   log_to_screen("Serious error(s) occurred already. I shan't try to write to media.");
         }
      else
         {
	   res =
	    eval_call_to_make_ISO (bkpinfo, bkpinfo->call_make_iso, bkpinfo->scratchdir,
	      ...
	      if (!run_program_and_log_output("tail -n10 /var/log/mondo-archive.log | fgrep ':-('", 1))
	        {
		  log_to_screen("Despite nonfatal errors, growisofs confirms the write was successful.");
		}
            }
          retval += res;
#ifdef DVDRWFORMAT
          sprintf(tmp, "tail -n8 %s | grep 'blank=full.*dvd-compat.*DAO'", MONDO_LOGFILE);
	  if (g_backup_media_type == dvd && (res || !run_program_and_log_output(tmp, 1)))
            {
	      log_to_screen("Failed to write to disk. I shall blank it and then try again.");
	      sleep(5);
	      system("sync");
...
// dvd+rw-format --- OPTION 2
	      if (!bkpinfo->please_dont_eject)
		{
		  ...
		  inject_device(bkpinfo->media_device);
              ...
              pause_for_N_seconds(5, "Letting DVD drive settle");
	      sprintf(sz_blank_disk, "dvd+rw-format %s", bkpinfo->media_device);
		  ...
		  log_to_screen("Warning - format failed. (Was it a DVD-R?) Sleeping for 5 seconds to take a breath...");
                  ...
                  res = run_external_binary_with_percentage_indicator_NEW("Blanking DVD disk", sz_blank_disk);
                  if (res) { log_to_screen("Format failed a second time."); }
		}
	      else { log_to_screen("Format succeeded. Sleeping for 5 seconds to take a breath..."); }
              pause_for_N_seconds(5, "Letting DVD drive settle");
	      if (!bkpinfo->please_dont_eject)
		{
		  log_to_screen("Ejecting media to clear drive status.");
		  ...
		  inject_device(bkpinfo->media_device);
              ...
              pause_for_N_seconds(5, "Letting DVD drive settle");
	      res = eval_call_to_make_ISO (bkpinfo, bkpinfo->call_make_iso, bkpinfo->scratchdir,
		   g_current_media_number, MONDO_LOGFILE, message_to_screen);
	      retval += res;
	      if (!bkpinfo->please_dont_eject)
		{
	      ...
	      if (res)
	        ...
	        { log_to_screen("OK, this time I successfully backed up to DVD."); }
	    }
#endif
...
// and add ' .' at end
		    ...
		    "mkisofs -no-emul-boot -b images/mindi-bootroot."IA64_BOOT_SIZE".img -c boot.cat -o _ISO_ -J -r -p MondoRescue -publisher www.mondorescue.org -A Mondo_Rescue_GPL -V _CD#_ .",
                    destfile, g_current_media_number, MONDO_LOGFILE,
                    message_to_screen);
#else
// FIXME --- change mkisofs string to MONDO_MKISOFS_REGULAR_SYSLINUX/LILO depending on bkpinfo->make_cd_usE_lilo
...
bool is_dev_an_NTFS_dev(char *bigfile_fname)
{
  ...
  malloc_string(tmp);
  malloc_string(command);
  sprintf(command, "dd if=%s bs=512 count=1 2> /dev/null | strings | head -n1", bigfile_fname);
  log_msg(1, "command = '%s'", command);
  strcpy(tmp, call_program_and_get_last_line_of_output(command));
  log_msg(1, "--> tmp = '%s'", tmp);
  if (strstr(tmp, "NTFS"))
    { iamhere("TRUE"); return(TRUE); }
  else
    { iamhere("FALSE"); return(FALSE); }
}
...
make_slices_and_images (struct s_bkpinfo *bkpinfo, char *biggielist_fname)
{
  ...
  open_progress_form ("Backing up big files", tmp,
		      ...
		      estimated_total_noof_slices);
  if (!(fin = fopen (biggielist_fname, "r"))) { log_OS_error("Unable to openin biggielist"); return(1); }
  for (fgets (bigfile_fname, MAX_STR_LEN, fin); !feof (fin);
      ...
      if (!does_file_exist (bigfile_fname))
	{
	  ...
	  sprintf (tmp, "bigfile %s was deleted - creating a dummy", bigfile_fname);
...
// Call partimagehack if it's a /dev entry (i.e. a partition to be imaged)
	  ...
	  use_partimagehack = FALSE;
	  if (!strncmp ( bigfile_fname, "/dev/", 5) && is_dev_an_NTFS_dev(bigfile_fname))
            {
	      use_partimagehack = TRUE;
	      log_msg (2, "Calling partimagehack in background because %s is an NTFS partition", bigfile_fname);
	      ...
	      partimagehack_fifo = sz_devfile;
	          ...
	          case 0:
		    log_msg (2, "CHILD - fip - calling feed_into_partimage(%s, %s)", bigfile_fname, sz_devfile);
		    res=feed_into_partimage(bigfile_fname, sz_devfile);
		    exit(res);
		    break;
                  default:
		    log_msg (2, "feed_into_partimage() called in background --- pid=%ld", (long int)(pid));
                }
	    }
...
// Whether partition or biggiefile, just do your thang :-)
	  sprintf (tmp, "Bigfile #%ld is '%s' (%ld KB)", biggie_file_number+1,
          ...
          if (IS_THIS_A_STREAMING_BACKUP(bkpinfo->backup_media_type))
	    {
	      write_header_block_to_stream (biggie_fsize, bigfile_fname,
					  use_partimagehack?BLK_START_A_PIHBIGGIE:BLK_START_A_NORMBIGGIE);
	    }
	  res =
	    slice_up_file_etc (bkpinfo, bigfile_fname, partimagehack_fifo,
	    	biggie_file_number, noof_biggie_files, use_partimagehack);
          if (IS_THIS_A_STREAMING_BACKUP(bkpinfo->backup_media_type))
	    {
	  ...
	  if (delete_when_done)
	    { unlink(bigfile_fname); delete_when_done=FALSE; }
	}
#ifndef _XWIN
      if (!g_text_mode) { newtDrawRootText (0, g_noof_rows - 2, tmp); newtRefresh (); }
#endif
    }
...
make_afioballs_and_images_OLD (struct s_bkpinfo *bkpinfo)
{
  ...
  sprintf (curr_filelist_fname, FILELIST_FNAME_RAW_SZ, bkpinfo->tmpdir, 0L);
  ...
  for (curr_set_no = 0; does_file_exist (curr_filelist_fname);
    ...
    {
      /* backup this set of files */
      sprintf (curr_filelist_fname,  FILELIST_FNAME_RAW_SZ,  bkpinfo->tmpdir, curr_set_no);
      sprintf (curr_afioball_fname, AFIOBALL_FNAME_RAW_SZ,bkpinfo->tmpdir,
               curr_set_no, bkpinfo->zip_suffix);
      
      log_msg (1, "EXAT'g set %ld", curr_set_no);
      sprintf (curr_xattr_list_fname, XATTR_LIST_FNAME_RAW_SZ, bkpinfo->tmpdir, curr_set_no);
      sprintf (curr_acl_list_fname,   ACL_LIST_FNAME_RAW_SZ,   bkpinfo->tmpdir, curr_set_no);
      ...
      res =
	archive_this_fileset (bkpinfo, curr_filelist_fname, curr_afioball_fname, curr_set_no);
      retval += res;
      if (res)
	{
	  sprintf (tmp,
...
// archives themselves
        ...
        {
          res = move_files_to_cd (bkpinfo, curr_filelist_fname, curr_xattr_list_fname, curr_acl_list_fname, curr_afioball_fname, NULL);
        }
      retval += res;
      g_current_progress++;
      update_progress_form (media_usage_comment);
  ...
  if (IS_THIS_A_STREAMING_BACKUP(bkpinfo->backup_media_type))
    {
      write_header_block_to_stream (0, "start-of-afioballs",
				  BLK_START_AFIOBALLS);
#if __FreeBSD__ == 5
      log_msg (1, "Using single-threaded make_afioballs_and_images() to suit b0rken FreeBSD 5.0");
...
make_those_slices_phase (struct s_bkpinfo *bkpinfo)
{
  ...
  sprintf (command, "cp %s %s/archives/", xattr_fname, bkpinfo->scratchdir);
  paranoid_system(command);
 ...
 */
int (*move_files_to_cd) (struct s_bkpinfo *, char *, ...) = _move_files_to_cd;
 ...
 */
int
_move_files_to_cd (struct s_bkpinfo *bkpinfo,
		 char *files_to_add, ...)
{
  ...
  if (retval)
    {
      log_msg (1, "Warning - errors occurred while I was adding files to CD dir");
    }
  paranoid_free(tmp);
  ...
  if (ask_me_OK_or_cancel ("About to write boot disk"))
    {
      ...
      if (ask_me_OK_or_cancel ("About to write 1.44MB mfsroot disk"))
        {
          ...
          if (format_first) { format_disk (datadisk_dev); }
	  ...
	  if (!does_file_exist(rootdisk_file) || !does_file_exist(bootdisk_file))
	    {
	      popup_and_OK("Cannot write boot/root floppies. Files not found.");
	      log_to_screen("Failed to find boot/root floppy images. Oh dear.");
	      return(1);
	    }
	  if (format_first) { format_disk (datadisk_dev); }
		/*
	  ...
	  if (ask_me_OK_or_cancel ("About to write root disk"))
	    {
	      ...
	      log_msg(1, "tmp = '%s'", tmp);
	      res += run_external_binary_with_percentage_indicator_NEW("Writing root floppy", tmp);
...
int res=0;
  ...
  assert(bkpinfo!=NULL);
	  ...
	  popup_and_OK ("Since you opted for a nonbootable backup, no boot floppies were created.");
      }
 ...
 * transfer files over the network) or leave it as is.
 */
int (*move_files_to_stream) (struct s_bkpinfo *, char *, ...) = _move_files_to_stream;
 ...
 */
int
_move_files_to_stream (struct s_bkpinfo *bkpinfo,
		 char *files_to_add, ...)
{
...
/* write closing header */
  ...
  if (retval)
    {
      log_msg (1, "Warning - errors occurred while I was adding file to tape");
    }
  paranoid_free(curr_file);
  return (retval);
}
  ...
  if (find_cdrw_device(cdrw_dev))
  ...
  else
    {
      if (!system("which cdrecord > /dev/null 2> /dev/null"))
        ...
        { cdrecord[0] = '\0'; log_msg(2, "Oh well. I guess I'll just pray then."); }
      if (cdrecord[0])
        {
         if (!keep_looping)
           {
             ...
             while((res=run_program_and_log_output(cdrecord, 5)))
               {
                 ...
                 if (ask_me_yes_or_no("Unable to examine CD. Are you sure this is a valid CD-R(W) CD?"))
                   { log_msg(1, "Well, he insisted..."); break; }
               }
            }
        }
    }
...
pause_and_ask_for_cdr (int ask_for_one_if_more_than_this, bool*pmountable)
{
  ...
  sprintf(szmsg, "I am about to burn %s #%d", media_descriptor_string(g_backup_media_type), g_current_media_number);
  log_to_screen(szmsg);
  if (g_current_media_number < ask_for_one_if_more_than_this)
    { return; }
 log_to_screen("Scanning CD-ROM drive...");
  sprintf(mtpt, "/tmp/cd.mtpt.%ld.%ld", (long int)random(), (long int)random());
...
*/
      ...
      if ((attempt_to_mount_returned_this=run_program_and_log_output(tmp, 1)))
        {
	 ...
	 */
        ...
        {
	  log_to_screen("%s has data on it but it's probably not a Mondo CD.", media_descriptor_string(g_backup_media_type));
        }
      else
	{
	  log_to_screen("%s found in drive. It's a Mondo disk.", media_descriptor_string(g_backup_media_type));
          cd_number = atoi(last_line_of_file(szcdno));
          sprintf(tmp, "cat %s 2> /dev/null", szserfname);
          strcpy(our_serial_str, call_program_and_get_last_line_of_output(tmp));
      ...
      log_msg(2, "our serial str = %s; g_serial_string = %s", our_serial_str, g_serial_string);
      if (cd_number > 0 && !strcmp(our_serial_str, g_serial_string))
        {
	  log_msg(2, "This %s is part of this backup set!", media_descriptor_string(g_backup_media_type));
          ok_go_ahead_burn_it = FALSE;
          if (cd_number == g_current_media_number-1)
            { log_to_screen("I think you've left the previous %s in the drive.", media_descriptor_string(g_backup_media_type)); }
          else
            { log_to_screen("Please remove this %s. It is part of the backup set you're making now.", media_descriptor_string(g_backup_media_type)); }
        }
      else
        {
    ...
    {
      log_msg(2, "paafcd: Can't find CD-ROM drive. Perhaps it has a blank %s in it?", media_descriptor_string(g_backup_media_type));
      if (interrogate_disk_currently_in_cdrw_drive(cdrw_dev, FALSE))
        {
          ok_go_ahead_burn_it = FALSE;
          log_to_screen("There isn't a writable %s in the drive.", media_descriptor_string(g_backup_media_type));
        }
    }
...
*/
  ...
  log_msg (2,"paafcd: OK, I assume I have a blank/reusable %s in the drive...", media_descriptor_string(g_backup_media_type));

  //  if (ask_for_one_if_more_than_this>1) { popup_and_OK(szmsg); }

  log_to_screen ("Proceeding w/ %s in drive.", media_descriptor_string(g_backup_media_type));
  paranoid_free(tmp);
  paranoid_free(szmsg);
 ...
 */
int
slice_up_file_etc (struct s_bkpinfo *bkpinfo, char *biggie_filename, char *partimagehack_fifo,
...
//  struct stat statbuf;
  ...
  if (partimagehack_fifo)
  ...
  else
    {
      ...
      if (!(fin = popen (command, "r"))) { log_OS_error("Unable to popen-in command"); return(1); }
  ...
  if (!(fin = fopen(file_to_openin, "r")))
    {
      ...
      sprintf (tmp, "Cannot archive bigfile '%s': not found", biggie_filename);
      log_to_screen (tmp);
  ...
  for (slice_num = 1; !finished; slice_num++)
    {
      ...
      if (!(fout = fopen (curr_slice_fname_uncompressed, "w"))) { log_OS_error(curr_slice_fname_uncompressed); return(1); }
      if ( (i == bkpinfo->optimal_set_size / 256) && (totalread < 1.1 * totallength) )
	{
	  ...
	  if (!does_file_exist (curr_slice_fname_uncompressed))
	    {
	      ...
	      sprintf (command, "%s -%d %s", bkpinfo->zip_exe,
		   bkpinfo->compression_level, curr_slice_fname_uncompressed);
    	      log_msg (2, command);
	      if ((res = system (command))) { log_OS_error(command); }
	  ...
	  if (res)
	  ...
	  else
	    {
	      sprintf (tmp,
		       ...
		       biggie_filename, biggie_file_number + 1, slice_num);
	    }
#ifndef _XWIN
	  if (!g_text_mode)
	    {
...
write_iso_and_go_on (struct s_bkpinfo *bkpinfo, bool last_cd)
{
  ...
  log_msg (1, "OK, time to make %s #%d", media_descriptor_string(bkpinfo->backup_media_type), g_current_media_number);
  ...
  if (last_cd)
    {
      unlink (lastcd_fname);
      log_msg (2, "OK, you're telling me this is the last CD. Fair enough.");
    }
  else
    {
  ...
  for (that_one_was_ok = FALSE; !that_one_was_ok;)
    {
      ...
      if (g_current_media_number==1 && !res && ( bkpinfo->backup_media_type == cdr || bkpinfo->backup_media_type == cdrw) )
        {
          if (find_cdrom_device(tmp, FALSE)) // make sure find_cdrom_device() finds, records CD-R's loc
            { log_msg(3, "*Sigh* Mike, I hate your computer."); bkpinfo->manual_cd_tray = TRUE; } // if it can't be found then force pausing
          else
	...
	{
	  log_to_screen
	    ("Please reboot from the 1st %s in Compare Mode, as a precaution.", media_descriptor_string(g_backup_media_type));
	  chdir ("/");
...
*/
  ...
  if (system (tmp))
    {
      log_msg
	(2, "Error occurred when I tried to delete the redundant IMGs and GZs");
    }
...
int verify_data(struct s_bkpinfo *bkpinfo)
{
  ...
  assert(bkpinfo!=NULL);  
	...
	//bkpinfo->backup_media_type == cdrw || bkpinfo->backup_media_type == cdr))
        {
          log_msg (2, "Not verifying again. Per-CD/ISO verification already carried out.");
          paranoid_system("cat /tmp/changed.files.* > /tmp/changed.files 2> /dev/null");
        }
      else
	{
	  ...
	  for (cdno = 1;
	       ...
	       cdno++)
	    {
	      if (cdno != g_current_media_number)
	        {
		  log_msg (2, "Warning - had to change g_current_media_number from %d to %d", g_current_media_number, cdno);
		  g_current_media_number = cdno;
		}		
	      if (bkpinfo->backup_media_type != iso)
		{
...
write_image_to_floppy_SUB (char *device, char *datafile)
{
  ...
  for (blockno = 0; blockno < maxblocks; blockno++)
      ...
      if (fread (blk, 1, 1024, fin) != 1024)
	{
	  if (feof(fin)) 
	    {
	      log_msg(1, "img read err - img ended prematurely - non-fatal error");
	      sleep(3);
	      return(res);
	    }
	  res++;
