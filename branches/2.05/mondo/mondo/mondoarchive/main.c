/***************************************************************************
    ...
    cvsid                : $Id: main.c,v 1.14 2004/06/21 20:20:36 hugo Exp $
 ***************************************************************************/
...
// for CVS
//static char cvsid[] = "$Id: main.c,v 1.14 2004/06/21 20:20:36 hugo Exp $";
...
{
  log_msg (0, "Mondo Archive v%s --- http://www.mondorescue.org", VERSION);
  log_msg (0, "running on %s architecture",get_architecture());
  log_msg (0,"-----------------------------------------------------------");
  log_msg (0, "NB: Mondo logs almost everything, so don't panic if you see");
  log_msg (0, "some error messages.  Please read them carefully before you");
  log_msg (0, "decide to break out in a cold sweat.    Despite (or perhaps");
  log_msg (0, "because of) the wealth of messages. some users are inclined");
  log_msg (0, "to stop reading this log. If Mondo stopped for some reason,");
  log_msg (0, "chances are it's detailed here.  More than likely there's a");
  log_msg (0, "message at the very end of this log that will tell you what");
  log_msg (0, "is wrong. Please read it!                          -Devteam");
  log_msg (0,"-----------------------------------------------------------");
...
void distro_specific_kludges_at_start_of_mondoarchive()
{
  ...
  run_program_and_log_output("umount `mount | grep shm | grep mondo | cut -d' ' -f3`", 2);
...
/* make sure PATH environmental variable allows access to mkfs, fdisk, etc. */
  ...
  if (strlen(tmp)>=MAX_STR_LEN-33)
    { fatal_error("Your PATH environmental variable is too long. Please shorten it."); }
...
//      chdir("/tmp");
  ...
  if (pre_param_configuration(bkpinfo)) { fatal_error("Pre-param initialization phase failed. Please review the error messages above, make the specified changes, then try again. Exiting..."); }
  ...
  if (argc==1)
    {
      ...
      if (res) { fatal_error("Syntax error. Please review the parameters you have supplied and try again."); }
    }
  else
    {
      res = handle_incoming_parameters (argc, argv, bkpinfo);
      if (res) {
        printf("Errors were detected in the command line you supplied.\n");
...
/* Finish configuring global structures */
  if (post_param_configuration (bkpinfo)) { fatal_error("Post-param initialization phase failed. Perhaps bad parameters were supplied to mondoarchive? Please review the documentation, error messages and logs. Exiting..."); }

  log_to_screen("BusyBox's sources are available from http://www.busybox.net");
  sprintf(g_erase_tmpdir_and_scratchdir, "rm -Rf %s %s", bkpinfo->tmpdir, bkpinfo->scratchdir);
  ...
  if (bkpinfo->verify_data)
    {
      ...
      if (res<0)
        {
          sprintf(tmp, "%d difference%c found.", -res, (-res != 1) ? 's' : ' ');
...
/* Report result of entire operation (success? errors?) */
  ...
  if (does_file_exist ("/root/images/mindi/mondorescue.iso"))
    {
      log_to_screen ("/root/images/mindi/mondorescue.iso, a boot/utility CD, is available if you want it.");
    }
  ...
  if (length_of_file ("/tmp/changed.files") > 2)
    {
      if (g_text_mode) { log_to_screen("Type 'less /tmp/changed.files' to see which files don't match the archives"); }
        ...
        {
          log_msg (1, "Type 'less /tmp/changed.files' to see which files don't match the archives");
  ...
  if (!g_cdrom_drive_is_here) { log_msg(10, "FYI, g_cdrom_drive_is_here was never used"); }
  if (!g_dvd_drive_is_here) { log_msg(10, "FYI, g_dvd_drive_is_here was never used"); }
