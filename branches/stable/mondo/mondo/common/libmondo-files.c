...
void store_nfs_config(struct s_bkpinfo *bkpinfo)
{
	...
	*(p++) = '\0';
    ...
    fprintf(fout, "    echo \"Making DHCP request on $ipdev\"\n");
    fprintf(fout, "    udhcpc -i $ipdev\n");
    fprintf(fout, "else\n");
    fprintf(fout, "    if [ \"$ipconf\" != \"\" ]; then\n");
    fprintf(fout, "        ipdev=`echo $ipconf | cut -d: -f1`\n");
    fprintf(fout, "        ipaddress=`echo $ipconf | cut -d: -f2`\n");
    fprintf(fout, "        ipnetmask=`echo $ipconf | cut -d: -f3`\n");
    fprintf(fout, "        ipbroadcast=`echo $ipconf | cut -d: -f4`\n");
    fprintf(fout, "        ipgateway=`echo $ipconf | cut -d: -f5`\n");
    fprintf(fout, "    fi\n");
